...
## Parity with the web (2026-09-08)

Every capability of the web dashboard is in the app, through `/api/mobile/*` routes that call the same server actions: Today (digest, next actions, bookings, the weekly strip, notifications), Inbox (Priority / All with category and channel filters, search, Gmail pull on refresh), the thread (draft intents, send, whole-conversation and per-message summaries, follow-up, stage, book from here with real slots, the tools menu: unread, archive, reclassify with a remembered rule, assign, relationship, delete), People (notes, relationship, merge decisions, client-portal invitation), Bookings (upcoming / past / canceled, status moves, confirm, reschedule with slots, cancel, assign a partner, questionnaire, delivery), Calendar, Assistant (proposals reviewed, edited, approved or dismissed; questions answered from the record), Automations (recipes, create, edit, delete, toggle), Business (profile, services, hours, booking link), Business memory, Channels (disconnect, retry, sync now, Apple Calendar with an app-specific password, a text number), Team (invite teammates and partners, revoke and resend, deactivate, partner access, join requests), Subscription (trial and checkout, billing portal — Stripe's pages in the browser), Profile (name, workspace, timezone, password, referral link), Notifications, Search, Workspaces (switch), Account (delete), forgot password.

Deliberately on the web: Google and Meta OAuth connections (a browser sign-in with the provider), the founder dashboard, the client portal and partner views.
