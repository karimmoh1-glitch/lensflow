...
import { generateInvitationToken, invitationExpiry } from "../src/lib/invitations";
...
async function main() {
  ...
  // A second organization the owner also belongs to — demonstrates the workspace switcher
  // and is used to verify cross-tenant isolation (this org's data must never leak into alex-photo).
  ...
  // ── Sarah: hot lead, graduation, asked to book, no reply yet ──────────────
  ...
  // ── Mike: warm lead, family session, asking price ──────────────────────────
  ...
  // ── Jessica: wedding inquiry, going cold ────────────────────────────────────
  ...
  // ── James: booked graduation, deposit pending confirmation ─────────────────
  ...
  // ── Priya: has portal access (CLIENT account), confirmed portrait session today,
  // outstanding card balance she can pay from the portal ──────────────────────
  ...
  // ── Diego: family shoot, tomorrow afternoon — assigned to partner Jordan ───
  ...
  // ── Emma: completed shoot, balance owed ─────────────────────────────────────
  ...
  // ── Lily: repeat client with membership ─────────────────────────────────────
  ...
  // ── Pending invitations — demonstrates the invite flow without accepting it ─
  ...
  console.log(`  Photographer: ${photographer.email} / ${PASSWORD}`);
  console.log(`  Partner:      ${partnerUser.email} / ${PASSWORD}`);
  console.log(`  Client:       ${priyaUser.email} / ${PASSWORD}`);
  console.log(`  Booking page: /book/${business.handle}`);
  console.log(`  Second org for the owner: ${otherBusiness.name} (${otherBusiness.handle}) — client "${otherClient.name}" must never be visible from alex-photo`);
}
