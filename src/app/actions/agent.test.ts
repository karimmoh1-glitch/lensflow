...
/**
 * The assistant's entitlement boundary and the automation count cap, exercised through
 ...
 */
describe("Assistant entitlement and automation caps", () => {
  const ids: string[] = [];
  ...
  it("Pro: allowed, with Pro's own hourly send cap", async () => {
    const { session } = await workspace("PRO");
    const brief = await getAgentBrief(session);
    expect(brief.allowed).toBe(true);
  });

  it("Pro with a lapsed subscription: denied (Free entitlements)", async () => {
    const { session } = await workspace("PRO", "CANCELED");
    const brief = await getAgentBrief(session);
    expect(brief.allowed).toBe(false);
    if (!brief.allowed) expect(brief.plan).toBe("FREE");
  });
