...
/**
 * The Business Agent's server boundary. Every entry point re-reads the plan from the
 * database and returns a structured denial for Free and Pro — the client can claim
 * whatever it likes; nothing here runs unless the row says Business and the subscription
 * is entitled. Staff only (owners, admins, team members); never clients or partners.
 */
const STAFF = ["OWNER", "ADMIN", "PHOTOGRAPHER"] as const;

export type AgentGate = { allowed: false; plan: "FREE" | "PRO"; reason: string };

function denial(plan: "FREE" | "PRO"): AgentGate {
  return { allowed: false, plan, reason: `The Daythread Business Agent is part of the Business plan. You're on ${PLANS[plan].name}.` };
}
