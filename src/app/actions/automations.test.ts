...
describe("automation editor actions", () => {
  ...
  it("Free: the first three start on, the fourth is saved switched off with the reason", async () => {
    ...
    for (let i = 0; i < 3; i++) expect((await create({ ...base, name: `A${i}` }, current.session)).paused).toBeUndefined();
    const fourth = await create({ ...base, name: "A3" }, current.session);
  ...
  it("Pro: uncapped", async () => {
    ...
    current.session = w.session;
    for (let i = 0; i < 5; i++) expect((await create({ ...base, name: `P${i}` }, current.session)).paused).toBeUndefined();
  });
