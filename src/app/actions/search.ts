...
export async function universalSearch(q: string): Promise<{ hits: SearchHit[]; summary?: { name: string; conversations: number; bookings: number; nextAction: string | null; href: string } }> {
  ...
  const [clients, conversations, bookings, automations] = await Promise.all([
    prisma.client.findMany({
      where: { businessId, OR: [{ name: contains }, { email: contains }, { phone: contains }, { instagram: contains }] },
      include: { _count: { select: { conversations: true, bookings: true } }, leads: { where: { respondedAt: null, status: { notIn: ["BOOKED", "LOST"] } }, select: { id: true, conversationId: true }, take: 1 }, bookings: { where: { startAt: { gte: new Date() }, status: { not: "CANCELED" } }, orderBy: { startAt: "asc" }, take: 1, include: { service: { select: { name: true } } } } },
      take: 5,
