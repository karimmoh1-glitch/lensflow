...
/**
 * Activates/deactivates a teammate's access — never deletes them, so their history
 * (assigned conversations, audit trail) stays intact. A deactivated membership is filtered
 * out of getUserMemberships(), so it's a real, enforced block on login/access, not just a
 * hidden UI row.
 */
