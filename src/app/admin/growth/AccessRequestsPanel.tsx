            ...
            <li key={r.id} className="py-3 flex flex-col gap-2 md:flex-row md:items-start md:gap-4">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="text-2xs text-ink/65">/{r.business.handle} · {r.business.planTier}</span>
                  <span className={cn("text-2xs font-bold uppercase tracking-[0.1em] rounded-full px-2 py-0.5", tone(r.status))}>{r.status.toLowerCase()}</span>
                </div>
              ...
              <div className="flex flex-col gap-1.5 shrink-0 md:w-64">
                <input value={notes[r.id] ?? ""} onChange={(e) => setNotes({ ...notes, [r.id]: e.target.value })} placeholder="Note to the workspace (optional)" maxLength={300} className="h-8 rounded-lg border border-border px-2.5 text-[12px] text-ink placeholder:text-ink/40 focus:outline-none focus:ring-2 focus:ring-ink/70" />
