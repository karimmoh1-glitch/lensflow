...
/**
 * The Gmail callback end to end with Google itself replaced by recorded responses: token
 * exchange and userinfo. Everything else — state verification, session/tenant binding,
 * encryption, the plan's channel allowance, redirects — is the real code against the real
 * database.
 */
...
const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
  ...
  if (url.startsWith("https://oauth2.googleapis.com/token")) return json({ access_token: "ya29.test-access", refresh_token: "1//test-refresh", expires_in: 3600, scope: "https://www.googleapis.com/auth/gmail.modify https://www.googleapis.com/auth/gmail.send https://www.googleapis.com/auth/userinfo.email", token_type: "Bearer" });
...
describe("Gmail callback", () => {
  ...
  beforeAll(async () => {
    ...
    businessId = (await prisma.business.create({ data: { name: "Gmail", handle: `gmail-${stamp}`, onboardingComplete: true } })).id;
    otherBusinessId = (await prisma.business.create({ data: { name: "Gmail Other", handle: `gmail-other-${stamp}`, onboardingComplete: true } })).id;
    userId = (await prisma.user.create({ data: { name: "Owner", email: `gmail-own-${stamp}@example.com`, passwordHash: "x" } })).id;
    strangerId = (await prisma.user.create({ data: { name: "Stranger", email: `gmail-stranger-${stamp}@example.com`, passwordHash: "x" } })).id;
  ...
  it("connects Gmail: state → session → tokens (encrypted) → Channels with connected=EMAIL", async () => {
    const { state, nonce } = await signOAuthStateRaw({ provider: "google", purpose: "gmail", businessId, userId });
    ...
    expect(loc.searchParams.get("tab")).toBe("channels");
    expect(loc.searchParams.get("connected")).toBe("EMAIL");
    const row = await prisma.integration.findUnique({ where: { businessId_provider: { businessId, provider: "EMAIL" } } });
    ...
    expect(calls.some((c) => c.includes("/calendar/"))).toBe(false);
  });

  it("an inbox still in onboarding is sent back to the connect step", async () => {
    await prisma.business.update({ where: { id: businessId }, data: { onboardingComplete: false } });
    const { state, nonce } = await signOAuthStateRaw({ provider: "google", purpose: "gmail", businessId, userId });
    cookieStore.nonce = nonce;
    session.current = { userId, activeBusinessId: businessId };
    const loc = location(await hit({ code: "auth-code", state }));
    expect(loc.pathname).toBe("/onboarding");
    expect(loc.searchParams.get("step")).toBe("connect");
    expect(loc.searchParams.get("connected")).toBe("EMAIL");
    await prisma.business.update({ where: { id: businessId }, data: { onboardingComplete: true } });
  });
  ...
  it("rejects an expired state", async () => {
    const { state, nonce } = await signOAuthStateRaw({ provider: "google", purpose: "gmail", businessId, userId, expiresIn: "1s" });
  ...
  it("refuses to finish a flow for another tenant or from another user's session", async () => {
    ...
    const { state: s2, nonce: n2 } = await signOAuthStateRaw({ provider: "google", purpose: "gmail", businessId: otherBusinessId, userId });
  ...
  it("enforces the plan's channel allowance at the callback: a full Free inbox is refused and the grant is revoked", async () => {
    // Fill Free's two slots with other channels, then finish a genuine Google flow.
    await prisma.integration.upsert({ where: { businessId_provider: { businessId, provider: "INSTAGRAM" } }, create: { businessId, provider: "INSTAGRAM", status: "CONNECTED", accessToken: "IGQVJ-x" }, update: { status: "CONNECTED", accessToken: "IGQVJ-x" } });
    ...
    await prisma.integration.updateMany({ where: { businessId, provider: "EMAIL" }, data: { status: "NOT_CONNECTED", accessToken: null, refreshToken: null } });
    session.current = { userId, activeBusinessId: businessId };
    const { state, nonce } = await signOAuthStateRaw({ provider: "google", purpose: "gmail", businessId, userId });
    cookieStore.nonce = nonce;
    calls.length = 0;
    ...
    expect(back.searchParams.get("provider")).toBe("EMAIL");
    expect(calls.some((c) => c.includes("oauth2.googleapis.com/revoke"))).toBe(true);
    const row = await prisma.integration.findUnique({ where: { businessId_provider: { businessId, provider: "EMAIL" } } });
    expect(row?.status).toBe("NOT_CONNECTED");
    expect(row?.refreshToken).toBeNull();
    await prisma.integration.deleteMany({ where: { businessId, provider: { in: ["INSTAGRAM", "SMS"] } } });
  });
