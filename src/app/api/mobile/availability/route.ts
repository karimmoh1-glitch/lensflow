...
export async function PUT(req: Request) {
  ...
  try { await saveAvailability(parsed.data.windows, session); return NextResponse.json({ ok: true }); } catch (e) { return jsonError(publicMessage(e, "Only an owner or admin can change hours."), 403); }
}
