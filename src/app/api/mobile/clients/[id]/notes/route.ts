...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try { await addClientNote(id, parsed.data.body, session); return NextResponse.json({ ok: true }); } catch (e) { return jsonError(publicMessage(e, "Couldn't save the note"), 400); }
}
