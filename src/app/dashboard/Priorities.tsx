    ...
    <section aria-labelledby="setup-label" className="mb-8 rounded-xl border border-border bg-white shadow-surface overflow-hidden">
      <div className="px-4 md:px-5 pt-4 pb-3 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5">
          <span className="text-xs text-ink/60 tabular-nums">{done} of {steps.length} done</span>
        ...
        {steps.map((s, i) => {
          ...
          return (
            <li key={s.key} className="grid grid-cols-[24px_minmax(0,1fr)] sm:grid-cols-[24px_minmax(0,1fr)_auto] items-start sm:items-center gap-x-3 gap-y-2 px-4 md:px-5 py-3">
              <span aria-hidden className={cn("w-6 h-6 rounded-full flex items-center justify-center shrink-0 text-2xs font-semibold tabular-nums", s.done ? "bg-ink text-white" : isNext ? "border border-ink/40 text-ink" : "border border-ink/15 text-ink/60")}>
                {s.done ? <Check className="w-3.5 h-3.5" strokeWidth={2.5} /> : i + 1}
              </span>
              <div className="min-w-0 flex-1">
                <p className={cn("text-13 font-semibold leading-5", s.done ? "text-ink/60" : "text-ink")}>
                  {s.title}
                  {s.done && <span className="sr-only"> (done)</span>}
                </p>
                {isNext && <p className="text-13 text-ink/60 leading-snug">{s.detail}</p>}
              </div>
              {!s.done && (
                <Link
                  href={s.href}
