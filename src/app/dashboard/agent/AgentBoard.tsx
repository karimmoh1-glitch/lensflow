...
import { MessageSquare, CalendarCheck, RotateCcw, CalendarX2, ArrowRight, Check, X } from "lucide-react";
...
const KIND: Record<ProposalKind, { icon: typeof MessageSquare; label: string; tone: string }> = {
  reply: { icon: MessageSquare, label: "Reply", tone: "bg-paper border border-border text-ink/70" },
  confirm_booking: { icon: CalendarCheck, label: "Confirm", tone: "bg-booking-soft text-booking-text" },
  follow_up: { icon: RotateCcw, label: "Follow up", tone: "bg-paper border border-border text-ink/70" },
  reconnect_calendar: { icon: CalendarX2, label: "Calendar", tone: "bg-warning-soft text-warning-text" },
};
...
export function AgentBoard({ initial }: { initial: { generatedAt: string; proposals: AgentProposal[]; activity: Activity[] } }) {
  ...
  const gate = (r: { allowed: boolean; reason?: string }) => {
    ...
    toast({ tone: "signal", title: "Daythread Pro required", body: (r as { reason: string }).reason });
    router.refresh();
    return true;
  };
  ...
  const counts = proposals.reduce<Record<ProposalKind, number>>((acc, p) => ({ ...acc, [p.kind]: (acc[p.kind] ?? 0) + 1 }), { reply: 0, confirm_booking: 0, follow_up: 0, reconnect_calendar: 0 });
  const noticed = [
  ...
  const card = (p: AgentProposal) => {
    ...
    return (
      <li key={p.id} className={cn("rounded-lg border bg-white transition-colors", isOpen ? "border-ink/25 shadow-popover" : "border-border")}>
        <div className="px-4 md:px-5 py-4 flex gap-3.5">
          <span className={cn("w-9 h-9 rounded-xl flex items-center justify-center shrink-0", k.tone)}><k.icon className="w-4 h-4" strokeWidth={1.75} aria-hidden /></span>
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-xs font-medium text-ink/65">{k.label}</span>
              {p.valueCents ? <span className="text-xs font-medium text-ink/65">· {formatMoney(p.valueCents)}</span> : null}
              <span className={cn("ml-auto text-xs font-medium rounded-md px-1.5 py-0.5", recommendationOnly ? "bg-ink/[0.05] text-ink/70" : isOpen ? "bg-ink/[0.03] text-ink/75" : "bg-success-soft text-success-text")}>{recommendationOnly ? "Recommendation" : isOpen ? "Preview · needs your approval" : "Ready with your approval"}</span>
            </div>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              {p.href && !recommendationOnly && <Link href={p.href} className="text-xs font-medium text-ink/70 hover:text-ink px-2 py-1">Open</Link>}
              <button type="button" onClick={() => dismiss(p)} disabled={pending} className="ml-auto inline-flex items-center gap-1 text-xs font-medium text-ink/65 hover:text-ink px-2 py-1 rounded-md hover:bg-ink/[0.05]"><X className="w-3.5 h-3.5" strokeWidth={1.75} aria-hidden /> Not now</button>
            </div>
          </div>
        </div>
        {isOpen && (
          <div className="border-t border-border px-4 md:px-5 py-4 bg-paper rounded-b-[22px]">
            <label htmlFor={`draft-${p.id}`} className="text-xs font-medium text-ink/65">Message{p.channel ? ` · ${p.channel.toLowerCase()}` : ""}</label>
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              <button type="button" onClick={() => setOpen(null)} className="text-xs font-medium text-ink/70 hover:text-ink px-2 py-1">Cancel</button>
              <span className="ml-auto text-xs text-ink/65">Goes out as you. Lands in the thread.</span>
            </div>
          </div>
        )}
      </li>
  ...
  return (
    <div className="space-y-8 dt-stagger">
      <AskAssistant />
      <section aria-label="What I noticed" className="rounded-lg border border-border bg-white px-5 py-4">
        <div className="text-xs font-medium text-ink/65">What I noticed</div>
        {noticed.length === 0 ? (
        ...
        <dl className="mt-3 grid grid-cols-3 gap-2 text-center">
          <div className="rounded-xl bg-paper px-2 py-2"><dt className="text-xs sm:text-13 font-medium text-ink/60 leading-tight">Ready to send</dt><dd className="text-lg font-semibold text-ink tabular-nums">{actionable.length}</dd></div>
          <div className="rounded-xl bg-paper px-2 py-2"><dt className="text-xs sm:text-13 font-medium text-ink/60 leading-tight">Suggested</dt><dd className="text-lg font-semibold text-ink tabular-nums">{recommendations.length}</dd></div>
          <div className="rounded-xl bg-paper px-2 py-2"><dt className="text-xs sm:text-13 font-medium text-ink/60 leading-tight">Done this week</dt><dd className="text-lg font-semibold text-ink tabular-nums">{initial.activity.filter((a) => a.result === "sent").length}</dd></div>
        </dl>
      </section>

      {actionable.length > 0 && (
        <section aria-label="What I recommend">
          <div className="flex items-baseline gap-3 mb-2.5 px-1"><h2 className="text-xs font-medium text-ink/65">What I&rsquo;d do next</h2><span className="text-xs text-ink/65">Review the message, then approve. Nothing goes out on its own.</span></div>
          <ul className="space-y-3">{actionable.map(card)}</ul>
        </section>
      )}
      {recommendations.length > 0 && (
        <section aria-label="Needs you">
          <div className="flex items-baseline gap-3 mb-2.5 px-1"><h2 className="text-xs font-medium text-ink/65">Needs you</h2><span className="text-xs text-ink/65">Things only you can fix.</span></div>
      ...
      <section aria-label="What I can handle" className="rounded-lg border border-border bg-paper px-5 py-4">
        <div className="text-xs font-medium text-ink/65">What I can handle</div>
        <ul className="mt-2 grid sm:grid-cols-2 gap-x-6 gap-y-1.5 text-sm text-ink/75">
          ...
          <li className="sm:col-span-2 text-ink/70">I never send without approval, never mark anything sent that wasn&rsquo;t delivered, and never touch your calendars directly — those I point you to.</li>
        </ul>
      </section>

      {initial.activity.length > 0 && (
        <section>
          <h2 className="text-xs font-medium text-ink/65 mb-2.5 px-1">Assistant activity · last 7 days</h2>
          <ul className="rounded-lg border border-border bg-white divide-y divide-border">
            {initial.activity.map((a, i) => (
              <li key={i} className="px-4 py-3 flex items-center gap-3 text-sm">
                ...
                <span className="text-xs text-ink/65 shrink-0">{a.result === "sent" ? "Sent" : a.result === "dismissed" ? "Dismissed" : a.result === "not_delivered" ? "Not delivered" : a.result}</span>
                <span className="text-xs text-ink/65 shrink-0" suppressHydrationWarning>{formatDistanceToNowStrict(new Date(a.at), { addSuffix: true })}</span>
              </li>
            ))}
          </ul>
        </section>
