        ...
        <div className="px-4 md:px-5 py-4 flex gap-3.5">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              {p.href && !recommendationOnly && <Link href={p.href} className="text-xs font-semibold text-ink/70 hover:text-ink px-2 py-1">Open</Link>}
              <button type="button" onClick={() => dismiss(p)} disabled={pending} className="ml-auto inline-flex items-center gap-1 text-xs font-semibold text-ink/65 hover:text-ink px-2 py-1 rounded-md hover:bg-black/[0.05]"><X className="w-3.5 h-3.5" strokeWidth={2} aria-hidden /> Not now</button>
            </div>
          </div>
        </div>
        {isOpen && (
