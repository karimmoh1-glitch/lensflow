        ...
        <div className="px-4 md:px-5 py-4 flex gap-3.5">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-ink/60">{k.label}</span>
              {p.valueCents ? <span className="text-[10px] font-bold uppercase tracking-[0.12em] text-ink/60">· {formatMoney(p.valueCents)}</span> : null}
              <span className={cn("ml-auto text-[10px] font-bold uppercase tracking-[0.1em] rounded-full px-1.5 py-0.5", recommendationOnly ? "bg-black/[0.05] text-ink/65" : isOpen ? "bg-signal-soft text-signal-text" : "bg-success-soft text-success-text")}>{recommendationOnly ? "Recommendation" : isOpen ? "Preview · needs your approval" : "Ready with your approval"}</span>
            </div>
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              {p.href && !recommendationOnly && <Link href={p.href} className="text-xs font-semibold text-ink/65 hover:text-ink px-2 py-1">Open</Link>}
              <button type="button" onClick={() => dismiss(p)} disabled={pending} className="ml-auto inline-flex items-center gap-1 text-xs font-semibold text-ink/60 hover:text-ink px-2 py-1 rounded-md hover:bg-black/[0.05]"><X className="w-3.5 h-3.5" strokeWidth={2} aria-hidden /> Not now</button>
            </div>
          </div>
        </div>
        {isOpen && (
          <div className="border-t border-border px-4 md:px-5 py-4 bg-paper/60 rounded-b-[22px]">
            <label htmlFor={`draft-${p.id}`} className="text-[11px] font-bold uppercase tracking-[0.12em] text-ink/60">Message{p.channel ? ` · ${p.channel.toLowerCase()}` : ""}</label>
            ...
            <div className="mt-3 flex items-center gap-2">
              ...
              <button type="button" onClick={() => setOpen(null)} className="text-xs font-semibold text-ink/65 hover:text-ink px-2 py-1">Cancel</button>
              <span className="ml-auto text-[11px] text-ink/60">Goes out as you. Lands in the thread.</span>
            </div>
          </div>
        )}
      </li>
    ...
    <div className="space-y-8 dt-stagger">
      ...
      <section aria-label="What I noticed" className="rounded-[22px] border border-border bg-white px-5 py-4">
        ...
        {noticed.length === 0 ? (
          ...
          <p className="mt-1.5 text-sm text-ink/80 leading-relaxed">{noticed.map((n, i) => <span key={n}>{i > 0 ? (i === noticed.length - 1 ? " and " : ", ") : ""}<span className="font-semibold text-ink">{n}</span></span>)}.{total > 0 ? <span className="text-ink/60"> About {formatMoney(total)} of work is riding on it.</span> : null}</p>
        )}
        <dl className="mt-3 grid grid-cols-3 gap-2 text-center">
          <div className="rounded-xl bg-paper px-2 py-2"><dt className="text-[10px] font-bold uppercase tracking-[0.1em] text-ink/60">Ready to send</dt><dd className="text-lg font-extrabold text-ink tabular-nums">{actionable.length}</dd></div>
          <div className="rounded-xl bg-paper px-2 py-2"><dt className="text-[10px] font-bold uppercase tracking-[0.1em] text-ink/60">Recommendations</dt><dd className="text-lg font-extrabold text-ink tabular-nums">{recommendations.length}</dd></div>
          <div className="rounded-xl bg-paper px-2 py-2"><dt className="text-[10px] font-bold uppercase tracking-[0.1em] text-ink/60">Done this week</dt><dd className="text-lg font-extrabold text-ink tabular-nums">{initial.activity.filter((a) => a.result === "sent").length}</dd></div>
        </dl>
      </section>

      {actionable.length > 0 && (
        <section aria-label="What I recommend">
          <div className="flex items-baseline gap-3 mb-2.5 px-1"><h2 className="text-[11px] font-bold uppercase tracking-[0.14em] text-ink/60">What I&rsquo;d do next</h2><span className="text-[11px] text-ink/60">Review the message, then approve. Nothing goes out on its own.</span></div>
          <ul className="space-y-3">{actionable.map(card)}</ul>
        </section>
      )}
      {recommendations.length > 0 && (
        <section aria-label="Needs you">
          <div className="flex items-baseline gap-3 mb-2.5 px-1"><h2 className="text-[11px] font-bold uppercase tracking-[0.14em] text-ink/60">Needs you</h2><span className="text-[11px] text-ink/60">Things only you can fix.</span></div>
      ...
      <section aria-label="What I can handle" className="rounded-[22px] border border-border bg-paper/60 px-5 py-4">
        <div className="text-[11px] font-bold uppercase tracking-[0.14em] text-ink/60">What I can handle</div>
        <ul className="mt-2 grid sm:grid-cols-2 gap-x-6 gap-y-1.5 text-sm text-ink/75">
          ...
          <li className="sm:col-span-2 text-ink/65">I never send without approval, never mark anything sent that wasn&rsquo;t delivered, and never touch your calendars directly — those I point you to.</li>
        </ul>
      </section>

      {initial.activity.length > 0 && (
        <section>
          <h2 className="text-[11px] font-bold uppercase tracking-[0.14em] text-ink/60 mb-2.5 px-1">Assistant activity · last 7 days</h2>
          <ul className="rounded-[22px] border border-border bg-white divide-y divide-border">
            {initial.activity.map((a, i) => (
              <li key={i} className="px-4 py-3 flex items-center gap-3 text-sm">
                ...
                <span className="text-xs text-ink/60 shrink-0">{a.result === "sent" ? "Sent" : a.result === "dismissed" ? "Dismissed" : a.result === "not_delivered" ? "Not delivered" : a.result}</span>
                <span className="text-xs text-ink/60 shrink-0" suppressHydrationWarning>{formatDistanceToNowStrict(new Date(a.at), { addSuffix: true })}</span>
              </li>
            ))}
          </ul>
        </section>
