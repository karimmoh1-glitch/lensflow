    ...
    <section aria-label="Ask the assistant" className="rounded-[22px] border border-border bg-white overflow-hidden">
      <div className="px-5 pt-4 pb-3">
        ...
        >
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Ask about your inbox, calendar or customers…" aria-label="Ask the assistant" maxLength={500} className="flex-1 bg-transparent text-[16px] md:text-sm text-ink placeholder:text-ink/60 outline-none min-w-0" />
        ...
        <ol className="border-t border-border divide-y divide-border" aria-live="polite">
          {history.map((h, i) => (
            <li key={i} className="px-5 py-3.5 dt-msg-in">
              <div className="text-xs font-semibold text-ink/65">{h.q}</div>
              <p className="mt-1 text-sm text-ink leading-relaxed whitespace-pre-wrap">{h.a}</p>
            </li>
          ))}
        </ol>
      )}
      <p className="px-5 py-2.5 border-t border-border text-[11px] text-ink/60">Answers come from your own records. The assistant never takes an action from here.</p>
    </section>
  );
}
