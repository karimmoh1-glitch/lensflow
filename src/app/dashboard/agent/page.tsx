...
import { ListChecks, MessageSquare, CalendarCheck, RotateCcw, Workflow, Lock } from "lucide-react";
      ...
      <div className="max-w-3xl mx-auto px-4 md:px-8 py-6 md:py-10">
        ...
        <section className="rounded-lg border border-border bg-white overflow-hidden">
          <div className="px-6 py-6 md:px-8 md:py-8 bg-[radial-gradient(120%_140%_at_0%_0%,rgba(109,90,230,0.12),transparent_55%)]">
            <span className="inline-flex items-center gap-1.5 text-xs font-medium text-ink/65"><Lock className="w-3 h-3" strokeWidth={2.5} aria-hidden /> Daythread Pro</span>
            <h2 className="mt-2 font-sans font-semibold text-[1.6rem] md:text-[1.9rem] leading-[1.05] tracking-[-0.03em] text-ink">{askedForIt ? "Meet your Business Agent." : "Your business has an assistant."}</h2>
            ...
            <ul className="mt-5 grid sm:grid-cols-2 gap-2.5">
              {[
                ...
                return (
                  <li key={i} className="flex items-start gap-2.5 rounded-lg border border-border bg-white/80 px-3.5 py-3 text-sm text-ink/80">
                    <span className="w-7 h-7 rounded-lg bg-ink text-ink/75 flex items-center justify-center shrink-0"><I className="w-3.5 h-3.5" strokeWidth={1.75} aria-hidden /></span>
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 md:px-8 py-6 md:py-10">
      <PageHeader
        ...
        action={<span className="inline-flex items-center gap-1.5 text-xs font-medium text-ink/75 bg-ink/[0.03] rounded-md px-2.5 py-1"><ListChecks className="w-3.5 h-3.5" strokeWidth={1.75} aria-hidden /> {PLANS[plan].name}</span>}
      />
