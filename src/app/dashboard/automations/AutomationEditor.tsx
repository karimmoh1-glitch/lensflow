...
const TRIGGER_LABEL: Record<Trigger, string> = { BOOKING_CREATED: "a booking is created", DEPOSIT_PAID: "a deposit is paid", DAYS_BEFORE_SHOOT: "a booking is coming up", SHOOT_COMPLETED: "a booking is completed", PAYMENT_DUE_SOON: "a payment is due", PAYMENT_OVERDUE: "a payment goes overdue", LEAD_INACTIVE: "a lead goes quiet" };
const ACTION_LABEL: Record<Action, string> = { SEND_CONFIRMATION: "send a confirmation", SEND_QUESTIONNAIRE: "send the questionnaire", SEND_REMINDER: "send a reminder", SEND_THANK_YOU: "send a thank-you", SEND_PAYMENT_REMINDER: "send a payment reminder", SEND_FOLLOW_UP: "send a follow-up" };
const TIMED: Trigger[] = ["DAYS_BEFORE_SHOOT", "SHOOT_COMPLETED", "PAYMENT_DUE_SOON", "PAYMENT_OVERDUE", "LEAD_INACTIVE"];
...
const RECIPES: Array<{ key: string; label: string; input: AutomationInput }> = [
  ...
  { key: "balance", label: "Chase the balance", input: { name: "Balance reminder", trigger: "PAYMENT_DUE_SOON", action: "SEND_PAYMENT_REMINDER", offsetHours: 72, messageTemplate: "Hi {{name}} — the remaining {{amount}} for your {{service}} on {{date}} is due before the session. Reply here if you've already sent it." } },
...
const VARS = ["name", "service", "date", "time", "amount", "business"];
...
export function EditAutomationButton({ automation }: { automation: { id: string } & AutomationInput }) {
  ...
  return (
    <>
      <button type="button" onClick={() => setOpen(true)} className="inline-flex items-center gap-1 text-xs font-semibold text-ink/50 hover:text-ink px-2 py-1 rounded-md hover:bg-black/[0.05]"><Pencil className="w-3.5 h-3.5" strokeWidth={2} aria-hidden />Edit</button>
...
function AutomationSheet({ open, onClose, existing }: { open: boolean; onClose: () => void; existing?: { id: string } & AutomationInput }) {
  ...
  return (
    <BottomSheet open={open} onClose={onClose} title={existing ? "Edit automation" : "New automation"} subtitle="When this happens → Daythread does this" icon={<Zap className="w-4 h-4 text-signal-text" strokeWidth={2} aria-hidden />} size="lg">
      {!existing && (
        <div className="mb-5">
          <div className="text-[11px] font-bold uppercase tracking-[0.12em] text-ink/45 mb-2">Start from</div>
      ...
      <div className="space-y-4">
        ...
        {timed && (
          <Field id="auto-offset" label={form.trigger === "DAYS_BEFORE_SHOOT" || form.trigger === "PAYMENT_DUE_SOON" ? "How long before" : "How long after"} hint="In hours. 24 = one day, 72 = three days.">
        ...
        <div className="flex items-center gap-2 pt-1">
          ...
          <button type="button" onClick={onClose} className="text-xs font-semibold text-ink/55 hover:text-ink px-2 py-1">Cancel</button>
          {existing && !confirmDelete && <button type="button" onClick={() => setConfirmDelete(true)} className="ml-auto inline-flex items-center gap-1 text-xs font-semibold text-ink/45 hover:text-danger-text px-2 py-1"><Trash2 className="w-3.5 h-3.5" strokeWidth={2} aria-hidden />Delete</button>}
          {existing && confirmDelete && <span className="ml-auto inline-flex items-center gap-1"><Button size="sm" variant="danger" onClick={remove} loading={pending} loadingLabel="Deleting">Delete</Button><button type="button" onClick={() => setConfirmDelete(false)} className="text-xs text-ink/50 px-2 py-1">Keep</button></span>}
        </div>
        <p className="text-[11px] text-ink/45">Sends on the channel the conversation lives on, or the client&rsquo;s email or phone. If that channel isn&rsquo;t connected, the run is recorded as not delivered — never pretended sent.</p>
      </div>
    </BottomSheet>
  );
}
