...
export function RescheduleCancel({ bookingId, canCancel, canReschedule, timezone, currentStartISO }: { bookingId: string; canCancel: boolean; canReschedule: boolean; timezone: string; currentStartISO: string }) {
  ...
  function cancel() {
    start(async () => {
      ...
      if (!r.ok) return setError(r.error);
      toast({ tone: "neutral", title: "Booking canceled", body: "Removed from your calendars. Payments and messages are untouched." });
      setOpen(null);
      router.refresh();
    });
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <BottomSheet open={open === "cancel"} onClose={() => setOpen(null)} title="Cancel this booking?" subtitle="It comes off your calendars. Nothing else changes." icon={<XCircle className="w-4 h-4 text-danger-text" strokeWidth={2} aria-hidden />}>
        <p className="text-sm text-ink/70 leading-relaxed">The time becomes bookable again. Payments already recorded stay as they are, and the conversation is kept. Let the customer know yourself — no message is sent automatically.</p>
      ...
      <p className="text-[10px] text-ink/40 mt-1">Current: {format(new Date(currentStartISO), "MMM d, h:mm a")}</p>
    </div>
  );
}
