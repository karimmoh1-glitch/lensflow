  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-8 py-6 md:py-10">
      <PageHeader
        ...
        action={<Link href="/dashboard/calendar" className="inline-flex items-center h-9 px-3.5 rounded-lg border border-border bg-white text-sm font-semibold text-ink hover:bg-black/[0.03]"><CalendarDays className="w-4 h-4 mr-1.5" strokeWidth={2} aria-hidden />Calendar</Link>}
      />
      ...
      <div className="flex items-center gap-1 mb-5 rounded-full bg-black/[0.04] p-1 w-fit" role="tablist" aria-label="Which bookings">
        {([["upcoming", "Upcoming", upcoming.length], ["past", "Past", pastCount], ["canceled", "Canceled", canceledCount]] as const).map(([key, label, n]) => (
          <Link key={key} href={key === "upcoming" ? "/dashboard/bookings" : `/dashboard/bookings?view=${key}`} role="tab" aria-selected={scope === key} className={cn("inline-flex items-center gap-1.5 h-8 px-3.5 rounded-lg text-13 font-semibold transition-colors", scope === key ? "bg-white text-ink shadow-xs" : "text-ink/70 hover:text-ink")}>
            {label} <span className={cn("text-2xs tabular-nums", scope === key ? "text-ink/65" : "text-ink/65")}>{n}</span>
          </Link>
        ))}
      </div>
                    ...
                    <li key={b.id}>
                      <Link href={`/dashboard/bookings/${b.id}`} className="flex items-center gap-3.5 px-4 md:px-5 py-3.5 hover:bg-black/[0.02] active:bg-black/[0.04] transition-colors">
                        <div className="w-12 shrink-0 text-center rounded-xl border border-border bg-paper py-1.5">
                          <div className="text-13 font-semibold text-ink/65 leading-none">{format(start, "EEE")}</div>
                          <div className="text-lg font-extrabold text-ink leading-tight tabular-nums">{format(start, "d")}</div>
                        </div>
                        <span className="hidden sm:flex w-8 h-8 rounded-full bg-accent-soft text-accent-text items-center justify-center text-2xs font-semibold shrink-0">{initials(b.client.name)}</span>
                        ...
                        <div className="text-right shrink-0">
                          ...
                          <div className="text-2xs text-ink/65">{b.service.durationMins} min</div>
                        </div>
