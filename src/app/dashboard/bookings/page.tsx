...
const STATUS: Record<string, { tone: "neutral" | "success" | "warning" | "info" | "danger"; label: string }> = {
  ...
  BALANCE_PAID: { tone: "success", label: "Paid in full" },
...
export default async function BookingsPage({ searchParams }: { searchParams: Promise<{ view?: string }> }) {
  ...
  const unconfirmed = upcoming.filter((b) => b.status === "BOOKED" || b.status === "DEPOSIT_PAID").length;
  const owed = upcoming.reduce((s, b) => s + Math.max(0, b.totalCents - b.payments.filter((p) => p.status === "PAID").reduce((x, p) => x + p.amountCents, 0)), 0);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-8 py-6 md:py-10">
      <PageHeader
        title="Bookings"
        description={upcoming.length === 0 ? "Nothing on the books yet." : `${upcoming.length} upcoming${unconfirmed ? ` · ${unconfirmed} not confirmed` : ""}${owed ? ` · ${formatMoney(owed)} still to collect` : ""}`}
            ...
            <section key={g.label} aria-label={g.label}>
              ...
              <ol className="rounded-[22px] border border-border bg-white divide-y divide-border overflow-hidden">
                {g.items.map((b) => {
                  ...
                  const due = Math.max(0, b.totalCents - paid);
                    ...
                    <li key={b.id}>
                      <Link href={`/dashboard/bookings/${b.id}`} className="flex items-center gap-3.5 px-4 md:px-5 py-3.5 hover:bg-black/[0.02] active:bg-black/[0.04] transition-colors">
                        ...
                        <div className="text-right shrink-0">
                          ...
                          <div className={cn("text-[11px] tabular-nums", due > 0 && b.status !== "CANCELED" ? "text-warning-text" : "text-ink/50")}>{due > 0 ? `${formatMoney(due)} due` : "Paid in full"}</div>
                        </div>
