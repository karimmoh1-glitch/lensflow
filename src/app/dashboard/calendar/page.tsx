    ...
    <div className="max-w-6xl mx-auto px-4 md:px-8 py-6 md:py-10">
      ...
      {attention.length > 0 && (
        <div role="alert" className="mb-4 rounded-2xl border border-warning/40 bg-warning-soft/60 px-4 py-3 text-sm text-ink/80 flex flex-wrap items-center gap-x-3 gap-y-1">
          ...
          <Link href="/dashboard/settings?tab=channels" className="text-signal-text font-semibold hover:underline">Fix in Settings →</Link>
        </div>
      )}

      {/* Week strip: seven big targets, the selected day solid. */}
      <div className="flex items-center gap-1 mb-4">
        <Link href={`?view=${view}&day=${dayKey(addDays(selected, -7))}`} aria-label="Previous week" className="w-9 h-9 shrink-0 inline-flex items-center justify-center rounded-md text-ink/65 hover:text-ink hover:bg-black/[0.05]"><ChevronLeft className="w-4 h-4" strokeWidth={2} /></Link>
            ...
            return (
              <li key={d.toISOString()}>
                <Link href={`?view=${view}&day=${dayKey(d)}`} aria-current={sel ? "date" : undefined} className={cn("flex flex-col items-center rounded-2xl py-2 min-h-[3.75rem] transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50", sel ? "bg-ink text-white" : "bg-white border border-border text-ink hover:bg-black/[0.03]")}>
                  <span className={cn("text-[10px] font-bold uppercase tracking-[0.1em]", sel ? "text-white/70" : "text-ink/60")}>{format(d, "EEE")}</span>
        ...
        <Link href={`?view=${view}&day=${dayKey(addDays(selected, 7))}`} aria-label="Next week" className="w-9 h-9 shrink-0 inline-flex items-center justify-center rounded-md text-ink/65 hover:text-ink hover:bg-black/[0.05]"><ChevronRight className="w-4 h-4" strokeWidth={2} /></Link>
      </div>
      ...
      <div className={cn("grid lg:grid-cols-[minmax(0,1fr)_320px] gap-6 mb-10", view === "week" && "hidden")}>
        ...
        <section aria-label="Day agenda" className="rounded-[22px] border border-border bg-white overflow-hidden">
          <div className="px-5 py-3.5 border-b border-border flex items-center justify-between gap-3">
            <h2 className="text-sm font-semibold text-ink">{isSelectedToday ? "Today" : format(selected, "EEEE")} <span className="text-ink/60 font-medium">· {format(selected, "MMM d")}</span></h2>
            <span className="text-xs text-ink/60">{agenda.items.length === 0 ? "Nothing scheduled" : `${agenda.items.length} ${agenda.items.length === 1 ? "item" : "items"}`}</span>
          </div>
          {agenda.items.length === 0 ? (
            <div className="px-5 py-10 text-center">
              ...
              <p className="mt-1 text-xs text-ink/65">{agenda.blocked ? "This date is blocked in Availability." : agenda.working.length === 0 ? "No working hours set for this weekday." : `Working ${agenda.working.map((w) => `${t(w.startAt)}–${t(w.endAt)}`).join(", ")}.`}</p>
            </div>
          ) : (
                  ...
                  {item.kind === "booking" ? (
                    <Link href={`/dashboard/bookings/${item.id}`} className="flex gap-4 px-5 py-3.5 hover:bg-black/[0.02] active:bg-black/[0.04]">
                      <div className="w-[4.5rem] shrink-0 text-right">
                        ...
                        <div className="text-[11px] text-ink/60 tabular-nums">{t(item.endAt)}</div>
                      </div>
                    ...
                    <div className="flex gap-4 px-5 py-3.5">
                      <div className="w-[4.5rem] shrink-0 text-right">
                        ...
                        {!item.allDay && <div className="text-[11px] text-ink/60 tabular-nums">{t(item.endAt)}</div>}
                      </div>
        ...
        <aside aria-label="Free time" className="rounded-[22px] border border-border bg-white">
          <div className="px-5 py-3.5 border-b border-border">
            ...
            <p className="text-[11px] text-ink/60 mt-0.5">Working hours minus bookings, buffers and busy calendar time.</p>
          </div>
          <div className="px-5 py-4">
            ...
            {agenda.calendars.length > 0 && (
              <p className="mt-4 text-[11px] text-ink/60">{agenda.calendars.map((c) => `${c.provider === "GOOGLE_CALENDAR" ? "Google" : "Apple"}${c.lastSyncedAt ? ` synced ${format(c.lastSyncedAt, "h:mm a")}` : c.status === "CONNECTED" ? " connected" : " needs attention"}`).join(" · ")}</p>
            )}
      ...
      <section aria-label="Month">
        ...
        <div className="grid grid-cols-7 text-[10px] md:text-xs font-semibold text-ink/60 mb-1.5">
