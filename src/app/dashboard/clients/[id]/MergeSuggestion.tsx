...
export function MergeSuggestion({ clientId, name, candidates }: { clientId: string; name: string; candidates: MergeCandidate[] }) {
  ...
  return (
    <section aria-labelledby="merge-title" className="mb-6 rounded-lg border border-border bg-ink/[0.03] px-4 py-3.5">
      <h2 id="merge-title" className="text-xs font-medium text-ink/65">Might be the same person</h2>
        ...
        {live.map((c) => (
          <li key={c.id} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4">
            ...
            <div className="flex gap-2 shrink-0">
              <button type="button" disabled={pending} onClick={() => act(c, true)} className="h-9 px-3.5 rounded-lg bg-ink text-white text-xs font-semibold disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink/70">Merge into {name.split(" ")[0]}</button>
              <button type="button" disabled={pending} onClick={() => act(c, false)} className="h-9 px-3.5 rounded-lg border border-border bg-white text-xs font-semibold text-ink/80 disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink/70">Not the same</button>
            </div>
          </li>
        ))}
      </ul>
