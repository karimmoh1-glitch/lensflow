...
export function RelationshipControls({ clientId, relationship, name }: { clientId: string; relationship: "LEAD" | "CUSTOMER" | "CONTACT"; name: string }) {
  ...
  return (
    <div>
      <div className="text-[11px] font-bold uppercase tracking-[0.12em] text-ink/60 mb-1.5">Relationship</div>
        ...
        {OPTIONS.map((o) => {
          ...
          return (
            <button
              ...
              className={cn("px-3 py-1 rounded-full text-xs font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50", on ? "bg-white text-ink shadow-xs" : "text-ink/65 hover:text-ink disabled:opacity-60")}
            >
              {o.label}
            </button>
          );
