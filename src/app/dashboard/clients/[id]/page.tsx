 ...
 * history as one thread — every conversation, booking and note in the order it
...
export default async function ClientDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const lastInteraction = [lastInbound && { when: lastInbound.createdAt, text: `${firstName(client.name)} wrote to you`, href: `/dashboard/inbox?c=${lastInbound.conversationId}` }, lastOutbound && { when: lastOutbound.createdAt, text: `You replied to ${firstName(client.name)}`, href: `/dashboard/inbox?c=${lastOutbound.conversationId}` }]
  ...
  const events: Event[] = [
    ...
    ...client.bookings.map((b) => ({
      when: b.startAt,
      kind: (b.status === "COMPLETED" || b.status === "FOLLOWED_UP" ? "outcome" : b.status === "CANCELED" ? "note" : "state") as ThreadKind,
        ...
        <section className="min-w-0">
          ...
          {events.length === 0 ? (
            <EmptyState
              ...
              description="Every conversation, booking and note with this person will build up here, in order."
            />
          ) : (
            <Card>
