...
export default async function ClientsPage() {
  ...
  const [clients, clientMemberships] = await Promise.all([
    prisma.client.findMany({
      ...
      include: { bookings: true, subscriptions: { where: { status: "ACTIVE" } }, conversations: { orderBy: { lastMessageAt: "desc" }, take: 1, select: { channel: true, lastMessageAt: true } } },
                ...
                >
                  ...
                  <div className="flex items-center flex-wrap gap-3 pl-11 sm:pl-0 shrink-0">
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-xs text-ink/60">{c.conversations[0] ? `Last on ${c.conversations[0].channel.toLowerCase()}` : "No conversation yet"}</div>
                    </div>
