        ...
        <div className="px-4 md:px-5 pt-3 md:pt-4 pb-3 border-b border-border space-y-3">
          ...
          <p className="text-sm text-ink" aria-live="polite">
            ...
            {view === "priority" && filteredOut > 0 && !q && (
              <span className="block text-[11px] text-ink/45 mt-0.5">{filteredOut} automated or promotional {filteredOut === 1 ? "message" : "messages"} kept out of the way · <Link href={href({ view: "all", cat: "all" })} className="underline decoration-ink/20 hover:text-ink">see all</Link></span>
            )}
          </p>
        ...
        {active ? (
          ...
          <div className="hidden lg:flex flex-1 items-center justify-center px-10">
            <div className="max-w-xs text-center">
              ...
              <p className="mt-1 text-sm text-ink/55 leading-relaxed">Who they are, what they mentioned and your history with them shows up beside it.</p>
            </div>
          </div>
        )}
      </div>
...
function Chip({ href, active, label, count, icon, tone }: { href: string; active: boolean; label: string; count?: number; icon?: React.ReactNode; tone?: "accent" }) {
  return (
    <Link href={href} aria-pressed={active} className={cn("inline-flex items-center gap-1.5 h-7 px-2.5 rounded-full font-medium whitespace-nowrap transition-all duration-150 shrink-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50", active ? "bg-ink text-white" : tone === "accent" && count ? "text-accent-text hover:bg-accent-soft" : "text-ink/70 hover:bg-black/[0.05]")}>
      {icon}
      {label}
      {typeof count === "number" && count > 0 && <span className={cn("tabular-nums", active ? "text-white/60" : "text-ink/35")}>{count}</span>}
    </Link>
  );
}
