...
export function CalendarSetup({ provider, mode, onDone, reconnect }: { provider: "GOOGLE_CALENDAR" | "APPLE_CALENDAR"; mode: "setup" | "manage"; onDone?: () => void; reconnect?: React.ReactNode }) {
  ...
  if (!state) return <div className="py-10 flex items-center justify-center text-sm text-ink/60"><WorkingDots /><span className="ml-2">Loading calendars</span></div>;
    ...
    <div className="space-y-5">
      ...
      <div>
        <div className="flex items-baseline justify-between gap-3">
          <h4 className="text-[11px] font-bold uppercase tracking-[0.14em] text-ink/60">Calendars to use with Daythread</h4>
          <button type="button" onClick={() => load(true)} disabled={pending} className="text-[11px] font-semibold text-ink/60 hover:text-ink">{busy === "refresh" ? "Refreshing…" : "Refresh list"}</button>
        </div>
        ...
        <ul className="mt-2 space-y-1.5">
              ...
              <li key={c.id} className={cn("rounded-xl border px-3.5 py-2.5 flex items-center gap-3 transition-colors", on ? "border-signal/40 bg-signal-soft/30" : "border-border bg-white hover:border-ink/20")}>
                <label className="flex items-center gap-3 flex-1 min-w-0 cursor-pointer">
                  ...
                  <span className="min-w-0">
                    <span className="block text-sm font-semibold text-ink truncate">{c.name}{c.primary ? <span className="ml-1.5 text-[10px] font-bold uppercase tracking-wide text-ink/60">primary</span> : null}</span>
                    <span className="block text-[11px] text-ink/60">{c.readOnly ? "Read-only · busy time only" : "Busy time blocks availability"}</span>
                  </span>
                </label>
                {on && !c.readOnly && (
                  <label className={cn("shrink-0 inline-flex items-center gap-1.5 text-[11px] font-semibold rounded-full px-2 py-1 cursor-pointer", bookingCalendar === c.id ? "bg-ink text-white" : "text-ink/65 hover:bg-black/[0.05]")}>
          ...
          {state.available.length === 0 && <li className="text-sm text-ink/65">No calendars found on this account.</li>}
        </ul>
        <p className="mt-2 text-[11px] text-ink/60">Daythread bookings stay the source of truth. Events on these calendars only block your availability; they never change a booking.</p>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {mode === "setup" && <button type="button" onClick={onDone} className="text-xs text-ink/60 px-2 py-1">Do this later</button>}
      </div>

      {mode === "manage" && (
        <dl className="grid grid-cols-2 gap-3 text-xs border-t border-border pt-4">
          <div><dt className="text-ink/60">Account</dt><dd className="font-semibold text-ink truncate">{state.account ?? "—"}</dd></div>
          <div><dt className="text-ink/60">Last synced</dt><dd className="font-semibold text-ink">{state.lastSyncedAt ? `${formatDistanceToNowStrict(new Date(state.lastSyncedAt))} ago` : "Not yet"}</dd></div>
          <div><dt className="text-ink/60">Busy blocks ahead</dt><dd className="font-semibold text-ink tabular-nums">{state.busyBlocks}</dd></div>
          <div><dt className="text-ink/60">Status</dt><dd className={cn("font-semibold", needsAttention ? "text-accent-text" : syncIssue ? "text-warning-text" : "text-success-text")}>{needsAttention ? "Needs attention" : syncIssue ? "Sync issue" : "Healthy"}</dd></div>
        </dl>
      )}
          ...
          <div className="mt-4">
            {!confirm ? (
              ...
              <div className="flex flex-wrap items-center gap-2 text-xs">
                ...
                <button type="button" onClick={() => setConfirm(false)} className="text-ink/60">Keep</button>
              </div>
            )}
          </div>
        </div>
