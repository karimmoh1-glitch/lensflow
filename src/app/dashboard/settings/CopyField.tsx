...
export function CopyField({ label, value }: { label: string; value: string }) {
  ...
  return (
    <div className="rounded-xl border border-border bg-paper/60 px-3 py-2">
      <div className="text-13 font-semibold text-ink/65">{label}</div>
      <div className="mt-1 flex items-center gap-2">
        <code className="min-w-0 flex-1 text-2xs font-mono text-ink/80 break-all">{value || "— set NEXT_PUBLIC_APP_URL —"}</code>
        <button
          type="button"
          onClick={copy}
          disabled={!value}
