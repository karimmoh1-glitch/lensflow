...
export function DangerZone({ businessName }: { businessName: string }) {
  ...
  return (
    <section aria-label="Delete inbox" className="mt-10 rounded-[22px] border border-danger/30 bg-white px-5 py-4">
      <h3 className="text-sm font-semibold text-ink">Delete this inbox</h3>
      <p className="mt-1 text-xs text-ink/60 leading-relaxed">Permanently removes {businessName}: people, conversations, messages, teammates&rsquo; access and every connected-account credential. Connected providers are told to stop where they support it, and any Daythread Pro subscription ends. This cannot be undone.</p>
      {!open ? (
        <div className="mt-3"><Button variant="outline" size="sm" onClick={() => setOpen(true)}>Delete inbox…</Button></div>
      ) : (
