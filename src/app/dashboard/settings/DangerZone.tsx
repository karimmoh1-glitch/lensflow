...
export function DangerZone({ businessName }: { businessName: string }) {
  ...
  return (
    <section aria-label="Delete workspace" className="mt-10 rounded-lg border border-danger/30 bg-white px-5 py-4">
      ...
      {!open ? (
        ...
        <div className="mt-3 space-y-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => setOpen(false)} className="text-xs text-ink/65">Cancel</button>
          </div>
        </div>
      )}
    </section>
