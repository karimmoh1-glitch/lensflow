...
export function InstagramDeliveryCheck({ initial }: { initial: DeliveryCheck | null }) {
  ...
  return (
    <div className="rounded-lg border border-border bg-paper px-3 py-2.5">
      ...
      {!check && !error && <p className="mt-1 text-xs text-ink/65">Asks Meta whether DMs to this account will actually arrive here, and shows where the chain breaks.</p>}
      {check && (
        <>
            ...
            {rows.map((r) => (
              <li key={r.label} className="flex gap-2 items-start">
                ...
                <span className="min-w-0">
                  ...
                  <span className="block text-xs text-ink/65 leading-snug">{r.detail}</span>
                </span>
              </li>
            ))}
          </ul>
          <p className="mt-2 text-xs text-ink/75 leading-relaxed border-t border-border pt-2">{check.verdict}</p>
