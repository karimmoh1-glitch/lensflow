  ...
  return (
    <li className="flex flex-wrap items-start gap-x-3 gap-y-1 py-2">
      ...
      <span className="min-w-0 flex-1">
        ...
        {setting.secret && <span className="ml-1.5 text-[10px] text-ink/65">secret — never shown</span>}
        <span className="block text-[11px] text-ink/70 leading-snug">{setting.label}</span>
        {setting.note && (
  ...
  return (
    <details className="rounded-[22px] border border-border bg-white overflow-hidden group">
      <summary className="cursor-pointer select-none list-none [&::-webkit-details-marker]:hidden px-5 py-4 flex items-center gap-3">
        ...
        <span className="shrink-0 text-[11px] font-semibold text-ink/65 group-open:hidden">Show</span>
        <span className="shrink-0 text-[11px] font-semibold text-ink/65 hidden group-open:inline">Hide</span>
      </summary>
      ...
      <div className="border-t border-border px-5 py-4 space-y-5 text-sm">
        ...
        {report.products.map((product) => (
          <section key={product.product}>
            <h4 className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.12em] text-ink/65">
              {product.name}
              <span className={`text-[10px] rounded-md px-1.5 py-0.5 ${product.ready ? "bg-success-soft text-success-text" : "bg-black/[0.05] text-ink/70"}`}>{product.ready ? "Ready to connect" : "Configuration required"}</span>
            </h4>
        ...
        <section>
          <h4 className="text-[11px] font-bold uppercase tracking-[0.12em] text-ink/65">Paste these into the Meta App Dashboard</h4>
        ...
        <p className="text-[11px] text-ink/65 flex items-start gap-1.5">
