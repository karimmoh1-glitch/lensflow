          ...
          <ol className="divide-y divide-border dt-rows">
            {rows.map((n) => (
              <li key={n.id} className={cn("flex items-start gap-3 px-4 py-3.5", !n.read && "bg-signal-soft/25")}>
                ...
                <time dateTime={n.createdAt.toISOString()} suppressHydrationWarning className="text-[11px] text-ink/45 shrink-0 tabular-nums">{formatDistanceToNowStrict(n.createdAt)} ago</time>
              </li>
            ))}
          </ol>
        </Card>
