...
export function RequestAccess({ provider, name, access, canAsk }: { provider: IntegrationProvider; name: string; access: AccessModel; canAsk: boolean }) {
  ...
  return (
    <div className="w-full sm:w-72 space-y-2">
      <textarea value={note} onChange={(e) => setNote(e.target.value)} maxLength={500} rows={2} aria-label="How you use it for your business (optional)" placeholder={`How you use ${name} for your business (optional)`} className="w-full rounded-lg border border-border px-3 py-2 text-13 text-ink placeholder:text-ink/60 focus:outline-none focus:ring-2 focus:ring-ink/70" />
