...
export function SmsNumberPicker({ current }: { current: string | null }) {
  ...
  if (current) {
    return (
      <div className="flex items-center gap-3 flex-wrap text-xs">
        <span className="text-ink/65">Your number</span>
        <span className="font-semibold text-ink tabular-nums">{current}</span>
        {!confirm ? (
          <button type="button" onClick={() => setConfirm(true)} className="text-ink/65 hover:text-ink">Release number</button>
        ) : (
          <span className="inline-flex items-center gap-2">
            <span className="text-ink/65">Texts to it will stop.</span>
  ...
  return (
    <div className="space-y-2">
      ...
      {numbers && numbers.length === 0 && !error && <p className="text-xs text-ink/70">No numbers found for that area code — try another.</p>}
        ...
        <ul className="grid sm:grid-cols-2 gap-1.5">
          {numbers.map((n) => (
            <li key={n.phoneNumber} className="flex items-center justify-between gap-2 rounded-lg border border-border bg-white px-3 py-2 text-xs">
              <span><span className="font-semibold text-ink tabular-nums">{n.friendlyName}</span><span className="text-ink/65"> · {[n.locality, n.region].filter(Boolean).join(", ")}</span></span>
