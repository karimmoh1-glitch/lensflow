  ...
  return (
    <div className="dt-stagger">
      ...
      {!subscriptionBillingIsLive && (
        <div className="text-sm text-ink/70 bg-black/[0.03] border border-border rounded-xl px-4 py-3 mb-6">
      ...
      {betaActive && current === "PRO" && business.planTier === "FREE" && (
        <div className="mb-6 rounded-xl border border-success/30 bg-success-soft/50 px-4 py-3.5 text-sm text-ink/80">
      ...
      {betaClaimable && (
        <div className="mb-6 rounded-xl border border-accent/30 bg-white px-4 py-4 flex flex-col sm:flex-row sm:items-center gap-3">
      ...
      {recommended && (
        <div className="mb-6 rounded-xl border border-border bg-black/[0.03] px-4 py-3.5 text-sm text-ink/80">
      ...
      {pastDue && (
        <div role="alert" className="mb-6 rounded-xl border border-warning/40 bg-warning-soft/60 px-4 py-3.5 flex flex-col sm:flex-row sm:items-center gap-3">
      ...
      {lapsed && (
        <div role="alert" className="mb-6 rounded-xl border border-danger/30 bg-danger-soft/50 px-4 py-3.5 text-sm text-ink/80">
      ...
      {(usage.overQuota || seatsOver || automationsOver) && (
        <div role="alert" className="mb-6 rounded-xl border border-warning/40 bg-warning-soft/60 px-4 py-3.5 text-sm text-ink/80 space-y-1">
      ...
      <section aria-label="Your Daythread subscription" className="mb-8 rounded-xl border border-border bg-white overflow-hidden">
        <div className="px-5 md:px-6 py-5 flex flex-col sm:flex-row sm:items-start justify-between gap-4">
          <div className="min-w-0">
            <p className="text-13 font-semibold text-ink/65">Your Daythread subscription</p>
      ...
      <section aria-label="Plans" className="mb-8">
        ...
        {(["month", "year"] as const).map((v) => (
          <Link key={v} href={`/dashboard/settings?tab=subscription&interval=${v}`} aria-current={interval === v ? "page" : undefined} className={cn("inline-flex items-center h-8 px-3 rounded-lg text-sm font-semibold transition-colors", interval === v ? "bg-ink text-white" : "text-ink/70 hover:text-ink")}>{v === "month" ? "Monthly" : "Yearly · 2 months free"}</Link>
        ))}
      </div>
      <div className="grid sm:grid-cols-3 gap-3 md:gap-4 items-stretch">
          {VISIBLE_PLANS.map((key: PlanKey) => {
            ...
            return (
              <div key={key} className={cn("rounded-xl border bg-white p-5 flex flex-col transition-colors", isCurrent ? "border-ink shadow-[0_18px_44px_-28px_rgba(16,17,20,0.5)]" : "border-border")}>
                ...
                {p.priceCents > 0 && interval === "year" && <p className="text-2xs font-semibold text-success-text">Two months free — {formatMoney(p.priceCents * 10 / 12)} a month, billed yearly</p>}
...
function Fact({ label, value, sub, tone }: { label: string; value: string; sub?: string; tone?: "warning" }) {
  return (
    <div className="bg-white px-5 py-3.5">
      <dt className="text-13 font-semibold text-ink/65">{label}</dt>
      <dd className={cn("mt-1 text-sm font-semibold", tone === "warning" ? "text-warning-text" : "text-ink")}>{value}</dd>
      {sub && <dd className="text-2xs text-ink/65 mt-0.5">{sub}</dd>}
    </div>
  );
}
