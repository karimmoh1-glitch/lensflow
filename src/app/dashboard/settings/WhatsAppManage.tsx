  ...
  return (
    <div className="space-y-3 text-xs">
      <dl className="grid grid-cols-[auto,1fr] gap-x-3 gap-y-1.5">
        <dt className="text-ink/50">Business account</dt>
        ...
        <dt className="text-ink/50">Webhooks</dt>
            ...
            {model.availableNumbers.map((n) => {
              ...
              return (
                <li key={n.id}>
                  ...
                  >
                    <Phone className="w-3.5 h-3.5 shrink-0 text-ink/45" strokeWidth={2} aria-hidden />
                    <span className="min-w-0 flex-1">
                      <span className="block font-semibold text-ink truncate">{n.displayPhoneNumber}</span>
                      <span className="block text-[11px] text-ink/55 truncate">{n.verifiedName}{n.codeVerificationStatus === "VERIFIED" ? " · verified" : ""}</span>
                    </span>
      ...
      <p className="text-[11px] text-ink/45">Disconnecting stops Meta delivering this number&rsquo;s events to Daythread and erases the stored credential. Your conversations and customers stay.</p>
    </div>
  );
}
