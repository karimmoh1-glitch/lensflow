  ...
  return (
    <div ref={ref} className="max-w-[1200px] mx-auto px-6">
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)] gap-10 lg:gap-16 items-center">
        <Reveal>
          <p className="text-2xs font-bold uppercase tracking-[0.16em] text-signal-text mb-4">Assistant</p>
          ...
          <p className="mt-5 text-ink/65 text-base max-w-sm">
        ...
        <div className="relative">
          <div className="rounded-2xl border border-border bg-white shadow-[0_32px_80px_-32px_rgba(16,17,20,0.3)] overflow-hidden" style={{ opacity: arrive, transform: `translateY(${(1 - arrive) * 12}px)` }}>
            <div className="px-5 py-4 border-b border-border flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-3 text-2xs font-semibold text-ink/65">
                    ...
                    <div className="px-5 py-3.5">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="text-2xs font-bold uppercase tracking-[0.12em] text-ink/65">{pr.kind}</div>
                          <div className="text-sm font-semibold text-ink truncate">{pr.title}</div>
                          <div className="text-xs text-ink/65 truncate">{pr.why}</div>
                        </div>
                        <span className="text-2xs font-bold uppercase tracking-wide text-success-text bg-success-soft px-2 py-1 rounded-md shrink-0 hidden sm:inline">Ready with your approval</span>
                      </div>
                      {first && (
                        <div className="overflow-hidden" style={{ maxHeight: `${open * 200}px`, opacity: open }}>
                          <div className="mt-3 ml-11 rounded-xl border border-border bg-paper px-3.5 py-3">
                            <div className="flex items-center gap-1.5 text-2xs font-bold uppercase tracking-[0.12em] text-ink/65 mb-1.5"><ChannelIcon k="instagram" size={14} />Draft · Instagram</div>
                            <p className="text-13 text-ink leading-relaxed">Hi Maya — Tuesday afternoon works. 2:00 PM? A brand session is $350 and runs about 90 minutes. I&rsquo;ll hold it for you.</p>
                          </div>
                          <div className="mt-2.5 ml-11 flex items-center gap-3">
                            <span className="relative inline-flex items-center justify-center h-9 px-4 rounded-full bg-ink text-white text-13 font-semibold overflow-hidden" style={{ transform: `scale(${1 - (approve > 0 && approve < 1 ? Math.sin(approve * Math.PI) * 0.04 : 0)})` }}>
                            ...
                            <span className="text-xs text-ink/65">Edit</span>
                            <span className="text-xs text-ink/65 ml-auto">Not now</span>
                          </div>
                        </div>
                      )}
                    </div>
            ...
            <div className="px-5 py-3 border-t border-border bg-paper/60">
              <div className="text-2xs font-bold uppercase tracking-[0.12em] text-ink/65 mb-1">Done today</div>
              <div className="flex items-center justify-between gap-3 overflow-hidden" style={{ maxHeight: `${done * 44}px`, opacity: done, transform: `translateY(${(1 - done) * -6}px)` }}>
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <span className="text-sm text-ink truncate">Replied to Maya Chen <span className="text-ink/65">· Instagram</span></span>
                </div>
                <span className="text-2xs font-bold text-success-text shrink-0">just now</span>
              </div>
              <p className="text-2xs text-ink/65 mt-1.5" style={{ opacity: 1 - done }}>Nothing here yet — approve something above.</p>
            </div>
          </div>
        </div>
      </div>
