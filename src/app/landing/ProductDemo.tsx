  ...
  return (
    <div className="relative max-w-[1200px] mx-auto px-6">
      ...
      <Reveal className="relative text-center max-w-2xl mx-auto mb-8">
        <h2 className="font-serif font-normal text-[clamp(2.4rem,4.8vw,3.9rem)] leading-[1.02] tracking-[-0.012em] text-ink">Try the inbox.</h2>
        ...
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              className={cn(
                "inline-flex items-center gap-2 h-9 px-3.5 rounded border text-13 font-medium transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink/70",
                on ? "bg-ink border-ink text-white" : "bg-white text-ink/75 border-border-strong hover:text-ink hover:bg-paper"
              )}
            >
      ...
      {/* The window */}
      <div ref={windowRef} className="dt-step relative rounded-[24px] border border-border bg-white shadow-elev-2 overflow-hidden grid grid-cols-1 md:grid-cols-[176px_minmax(0,1fr)] min-h-[460px] transition-shadow duration-700" style={{ boxShadow: `0 40px 100px -40px ${active.tint}66, 0 2px 6px rgba(16,17,20,0.05)`, "--p": 1, "--a": 0, "--b": 0.35, "--dy": "40px", "--ds": 0.03 } as CSSProperties}>
        {/* Sidebar */}
        <aside className="hidden md:flex flex-col border-r border-border bg-paper/70 py-4 px-3 gap-0.5">
          <div className="flex items-center gap-2 px-2 pb-4">
            ...
            <span className="text-sm font-semibold tracking-tight text-ink">Daythread</span>
          </div>
          {[
            ...
            return (
              <button
                ...
                className={cn("flex items-center gap-2.5 rounded px-2 h-8 text-13 font-medium text-left transition-colors duration-150", on ? "bg-ink/[0.07] text-ink" : clickable ? "text-ink/70 hover:bg-ink/[0.04]" : "text-ink/65 cursor-default")}
              >
                <i.icon className="w-4 h-4 shrink-0 text-ink/55" strokeWidth={1.75} />
                {i.label}
                {i.n && <span className="ml-auto text-xs tabular-nums text-ink/65">{i.n}</span>}
              </button>
            );
          })}
        </aside>
        ...
        <div key={tab} className="dt-swap min-w-0">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_270px] h-full">
              <div className="divide-y divide-border">
                ...
                {CONVOS.map((x, i) => {
                  const on = x.id === convo;
                  return (
                    <button key={x.id} type="button" onClick={() => pick(x.id)} aria-current={on ? "true" : undefined} className={cn("dt-step w-full text-left px-5 py-3.5 flex gap-3 transition-colors duration-200", on ? "bg-ink/[0.04]" : "hover:bg-ink/[0.02]")} style={{ "--a": 0.2 + i * 0.07, "--b": 0.38 + i * 0.07, "--dy": "10px" } as CSSProperties}>
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className={cn("block text-2xs font-bold mt-0.5", x.tagTone)}>{x.tag}</span>
                      </span>
                    </button>
                  );
                })}
              ...
              <div key={convo} className="dt-swap border-t lg:border-t-0 lg:border-l border-border bg-paper/60 p-5">
                ...
                <div className="mt-4 flex items-center gap-3">
                  <span className="w-9 h-9 rounded-full bg-ink/[0.05] text-ink/70 flex items-center justify-center text-2xs font-semibold">{c.name.split(" ").map((p) => p[0]).join("")}</span>
          ...
          {tab === "client" && (
            <div className="p-5 md:p-6">
              <div className="flex items-center gap-4 mb-5">
                ...
                <div className="min-w-0"><div className="text-lg font-semibold text-ink tracking-tight">Maya Chen</div><div className="text-xs text-ink/65">maya@hey.com · @maya.makes</div></div>
                <div className="ml-auto text-right"><div className="text-2xs text-ink/70">Bookings</div><div className="text-xl font-semibold text-ink tabular-nums">2</div></div>
              </div>
              ...
              <ol className="relative pl-7">
                ...
                {[
                  ...
                  <li key={title + when} className="relative py-2 hover:bg-ink/[0.02] -mx-2 px-2 rounded-lg transition-colors">
                    <span aria-hidden className={cn("absolute -left-5 top-[15px] w-[15px] h-[15px] rounded-full border-[3px] border-white", dot)} />
                    <div className="flex justify-between gap-3"><div className="min-w-0"><div className="text-sm font-medium text-ink truncate">{title}</div><div className="text-xs text-ink/65 truncate">{meta}</div></div><div className="text-2xs text-ink/65 tabular-nums shrink-0">{when}</div></div>
                  </li>
                ))}
              </ol>
            </div>
          ...
          {tab === "booking" && (
            <div className="p-5 md:p-6">
              <div className="flex items-start justify-between gap-4 mb-5">
                <div><div className="text-2xs font-medium text-ink/60">Booking</div><div className="text-lg font-semibold text-ink tracking-tight mt-1">Brand session · Maya Chen</div><div className="text-sm text-ink/70">Tue, Sep 9 · 2:00 – 4:00 PM · Studio</div></div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-[1fr_220px] gap-5">
                ...
                <div className="rounded-2xl border border-border bg-paper/60 p-4">
                  ...
                  <button type="button" className="mt-3 w-full h-9 rounded bg-ink text-white text-xs font-semibold transition-transform">Reschedule</button>
                </div>
              </div>
            </div>
          )}
          ...
          {tab === "calendar" && (
            <div className="p-5 md:p-6">
              <div className="flex items-center justify-between gap-3 mb-4">
                <div><div className="text-2xs font-medium text-ink/60">Today</div><div className="text-lg font-semibold text-ink tracking-tight mt-0.5">Tuesday, Sep 9</div></div>
              ...
              <ol className="rounded-2xl border border-border divide-y divide-border overflow-hidden">
                {[
                  ...
                  <li key={String(who)} className={cn("flex items-center gap-4 px-4 py-3 transition-colors", booking ? "hover:bg-ink/[0.02]" : "bg-paper/50")}>
                    <div className="w-14 shrink-0 text-right"><div className="text-sm font-semibold text-ink tabular-nums">{String(s)}</div><div className="text-2xs text-ink/65 tabular-nums">{String(e)}</div></div>
                    ...
                    {booking && <span className="text-2xs font-bold text-ink/65">Open thread →</span>}
                  </li>
                ))}
              </ol>
          ...
          {tab === "ai" && (
            <div className="p-5 md:p-6 flex flex-col min-h-[400px]">
              <div className="flex items-center gap-2 mb-4"><span className="w-7 h-7 rounded-full bg-ink/[0.05] text-ink/70 flex items-center justify-center"><ListChecks className="w-3.5 h-3.5" strokeWidth={2} /></span><span className="text-sm font-semibold text-ink">Assistant</span><span className="text-2xs text-ink/65">knows your whole thread</span></div>
              <div className="flex flex-wrap gap-2 mb-4">
                {["What do I need to do today?", "What's on my calendar this week?", "Which inquiries are going cold?"].map((q) => (
                  <button key={q} type="button" onClick={() => setAsked(q)} className={cn("text-xs px-3 py-1.5 rounded-full border transition-all duration-200 active:scale-95", asked === q ? "bg-ink text-white border-ink" : "border-border hover:bg-ink/[0.04] hover:-translate-y-px")}>{q}</button>
                ))}
              </div>
              {asked && (
                <div key={asked} className="dt-swap space-y-3 max-w-lg">
                  ...
                  <div className="flex items-start gap-2">
                    <span className="w-6 h-6 rounded-full bg-ink/[0.05] text-ink/70 flex items-center justify-center shrink-0 mt-0.5"><ListChecks className="w-3 h-3" strokeWidth={2} /></span>
...
const BEAT = {
  ...
  thinking: { dot: "bg-ink/50", label: "text-ink/70", bg: "bg-ink/[0.03]" },
