          ...
          {[
            ...
            return (
              <button
                ...
                className={cn("flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-13 font-semibold text-left transition-all duration-200", on ? "bg-ink text-white" : clickable ? "text-ink/70 hover:bg-black/[0.04] hover:translate-x-0.5" : "text-ink/65 cursor-default")}
              >
                <i.icon className="w-4 h-4 shrink-0" strokeWidth={2} />
                {i.label}
                {i.n && <span className={cn("ml-auto text-2xs font-bold rounded-full px-1.5", on ? "bg-white/20 text-white" : "bg-accent-strong text-white")}>{i.n}</span>}
              </button>
            );
          })}
        </aside>
