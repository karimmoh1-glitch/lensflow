...
import { Inbox, Users, CalendarClock, Zap, Home, CreditCard } from "lucide-react";
import { cn } from "@/lib/utils";

/**
 * A miniature Daythread you can click. Four surfaces, each built from the product's own
 * grammar: the inbox with its context rail, a client as a relationship timeline, a booking
 * with its money attached, and automations that read as WHEN → IF → THEN and actually
 * toggle. Nothing here is a screenshot.
 */
type Tab = "inbox" | "client" | "booking" | "automation";

const TABS: { key: Tab; label: string; icon: typeof Inbox; hint: string }[] = [
  { key: "inbox", label: "Inbox", icon: Inbox, hint: "Click a conversation — the context appears." },
  { key: "client", label: "Client", icon: Users, hint: "A client is a history, not a row." },
  { key: "booking", label: "Booking", icon: CalendarClock, hint: "The booking carries its own money." },
  { key: "automation", label: "Automations", icon: Zap, hint: "Turn one on. It reads like a sentence." },
];

const CONVOS = [
  { id: "maya", name: "Maya Chen", ch: "Instagram", chBg: "bg-gradient-to-br from-[#FEDA75] via-[#D62976] to-[#4F5BD5]", msg: "Hey! Are you free Tuesday afternoon?", when: "2h", tag: "Needs reply", tagTone: "text-accent-text", score: 82, status: "Returning client", ltv: "$2,150", mention: [["Service", "Brand session"], ["Date", "Tuesday PM"]], history: ["Brand session · Mar · paid $700", "Headshots · Nov · paid $350"] },
  { id: "jordan", name: "Jordan Lee", ch: "Gmail", chBg: "bg-[#EA4335]", msg: "Following up on pricing for a September date.", when: "9d", tag: "Going cold", tagTone: "text-warning-text", score: 61, status: "Warm lead", ltv: "—", mention: [["Service", "Full package"], ["Date", "September"]], history: ["First inquiry · 9 days ago"] },
  { id: "priya", name: "Priya Patel", ch: "Messenger", chBg: "bg-gradient-to-br from-[#00B2FF] to-[#8134F5]", msg: "Loved the preview! Can we book the full package?", when: "1d", tag: "Ready to book", tagTone: "text-success-text", score: 91, status: "Client · booked", ltv: "$1,380", mention: [["Service", "Full package"], ["Balance", "$175 open"]], history: ["Full session · today 3pm", "Deposit $1,380 · paid"] },
];
  ...
  return (
    <div className="max-w-6xl mx-auto px-6">
      <div className="text-center max-w-2xl mx-auto mb-8 md:mb-10">
        <p className="text-[12px] font-bold uppercase tracking-[0.14em] text-ink/45 mb-4">Try it</p>
        <h2 className="font-sans font-extrabold text-[clamp(2.2rem,5vw,4rem)] leading-[0.96] tracking-[-0.04em] text-ink text-balance">Click around.</h2>
      </div>

      <div className="flex flex-wrap items-center justify-center gap-2 mb-6" role="tablist" aria-label="Product surfaces">
        {TABS.map((t) => {
          const on = t.key === tab;
          return (
            <button
              ...
              className={cn(
                "inline-flex items-center gap-2 h-10 px-4 rounded-full text-sm font-bold transition-all duration-200 active:scale-95 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/50",
                on ? "bg-ink text-white shadow-sm" : "bg-white text-ink/70 border border-border hover:text-ink hover:border-ink/20"
              )}
            >
      ...
      <p className="text-center text-sm text-ink/55 mb-6" aria-live="polite">{active.hint}</p>

      {/* The window */}
      <div className="rounded-[22px] border border-border bg-white shadow-[0_32px_80px_-32px_rgba(16,17,20,0.3),0_2px_6px_rgba(16,17,20,0.05)] overflow-hidden grid grid-cols-1 md:grid-cols-[168px_minmax(0,1fr)] min-h-[440px]">
        {/* Sidebar */}
          ...
          {[
            ...
            return (
              <button
                ...
                className={cn("flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-[13px] font-semibold text-left transition-colors", on ? "bg-ink text-white" : clickable ? "text-ink/70 hover:bg-black/[0.04]" : "text-ink/40 cursor-default")}
              >
        ...
        {/* Main */}
        <div key={tab} className="min-w-0 animate-[fadeUp_0.3s_cubic-bezier(0.16,1,0.3,1)]">
          {tab === "inbox" && (
            <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_260px] h-full">
              <div className="divide-y divide-border">
                ...
                {CONVOS.map((x) => {
                  const on = x.id === convo;
                  return (
                    <button key={x.id} type="button" onClick={() => setConvo(x.id)} className={cn("w-full text-left px-5 py-3.5 flex gap-3 transition-colors", on ? "bg-accent-soft/40" : "hover:bg-black/[0.02]")}>
                      <span className={cn("w-9 h-9 rounded-full shrink-0 flex items-center justify-center text-white text-[11px] font-bold", x.chBg)}>{x.name.split(" ").map((p) => p[0]).join("")}</span>
                      <span className="min-w-0 flex-1">
                        <span className="flex items-center gap-2"><span className="text-sm font-semibold text-ink truncate">{x.name}</span><span className="text-[10px] text-ink/45">· {x.ch} · {x.when}</span></span>
              ...
              <div key={convo} className="border-t lg:border-t-0 lg:border-l border-border bg-paper/60 p-5 animate-[fadeUp_0.3s_ease-out]">
          ...
          {tab === "client" && (
            <div className="p-5 md:p-6">
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-accent/30 bg-gradient-to-br from-accent-soft/70 to-transparent px-4 py-3 mb-5">
                <span className="w-2 h-2 rounded-full bg-accent shrink-0" /><div className="min-w-0 flex-1"><div className="text-sm font-semibold text-ink">Reply to Maya</div><div className="text-xs text-ink/65">Waiting 2 hours · $350 opportunity</div></div><span className="text-xs font-bold text-accent-text">Reply →</span>
              </div>
              ...
              <div className="grid sm:grid-cols-[1fr_220px] gap-5">
                <div>
                  ...
                  <ol className="relative pl-6 space-y-2 text-sm">
                    ...
                    <li className="relative"><span aria-hidden className="absolute -left-6 top-[5px] w-[11px] h-[11px] rounded-full border-2 border-white bg-accent" />Maya: “Running 10 late!!” <span className="text-ink/45 text-xs">· SMS</span></li>
          ...
          {tab === "automation" && (
            <div className="p-5 md:p-6 space-y-3">
              {[
                ...
                const on = autos[a.k];
                return (
                  <div key={a.k} className={cn("rounded-2xl border border-border p-4 transition-opacity", !on && "opacity-60")}>
