  ...
  return (
    <div className="relative max-w-[1200px] mx-auto px-6">
      ...
      <Reveal className="relative text-center max-w-2xl mx-auto mb-8">
        <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-ink/60 mb-4">Try it</p>
        ...
        <p className="mt-4 text-ink/65 text-base">The real interface. Click a conversation and the context appears beside it.</p>
      </Reveal>
      ...
      <p className="relative text-center text-sm text-ink/65 mb-6 h-5" aria-live="polite">{active.hint}</p>
          ...
          {[
            ...
            return (
              <button
                ...
                className={cn("flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-[13px] font-semibold text-left transition-all duration-200", on ? "bg-ink text-white" : clickable ? "text-ink/70 hover:bg-black/[0.04] hover:translate-x-0.5" : "text-ink/60 cursor-default")}
              >
        ...
        <div key={tab} className="dt-swap min-w-0">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_270px] h-full">
              <div className="divide-y divide-border">
                <div className="px-5 py-3 flex items-center gap-2">
                  ...
                  <span className="text-[11px] text-ink/60">Sorted by what needs you</span>
                  <span className="ml-auto hidden lg:inline-flex items-center gap-1 text-[10px] text-ink/60"><kbd className="rounded border border-border bg-paper px-1 font-sans">↑</kbd><kbd className="rounded border border-border bg-paper px-1 font-sans">↓</kbd> to move</span>
                </div>
                {CONVOS.map((x) => {
                  const on = x.id === convo;
                  return (
                    <button key={x.id} type="button" onClick={() => setConvo(x.id)} className={cn("w-full text-left px-5 py-3.5 flex gap-3 transition-all duration-200", on ? "bg-accent-soft/40" : "hover:bg-black/[0.02] hover:translate-x-0.5")}>
                      ...
                      <span className="min-w-0 flex-1">
                        <span className="flex items-center gap-2"><span className="text-sm font-semibold text-ink truncate">{x.name}</span><span className="text-[10px] text-ink/60">· {x.when}</span></span>
              ...
              <div key={convo} className="dt-swap border-t lg:border-t-0 lg:border-l border-border bg-paper/60 p-5">
                ...
                <div className="mt-3 flex items-center gap-2 text-[11px]"><span className="rounded-full bg-success-soft text-success-text font-bold px-2 py-0.5">{c.tag}</span><span className="text-ink/65">{c.ltv}</span></div>
                <div className="mt-4 text-[10px] font-bold uppercase tracking-[0.12em] text-ink/60">They mentioned</div>
                <dl className="mt-1.5 space-y-1 text-xs">{c.mention.map(([k, v]) => <div key={k} className="flex justify-between"><dt className="text-ink/65">{k}</dt><dd className="font-semibold text-ink">{v}</dd></div>)}</dl>
                <div className="mt-4 text-[10px] font-bold uppercase tracking-[0.12em] text-ink/60">History</div>
          ...
          {tab === "client" && (
            <div className="p-5 md:p-6">
              <div className="flex items-center gap-4 mb-5">
                ...
                <div className="ml-auto text-right"><div className="text-[11px] text-ink/65">Bookings</div><div className="text-xl font-extrabold text-ink tabular-nums">2</div></div>
              </div>
              ...
              <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-ink/60 mb-2">Relationship</div>
              <ol className="relative pl-7">
                ...
                {[
                  ...
                  <li key={title + when} className="relative py-2 hover:bg-black/[0.02] -mx-2 px-2 rounded-lg transition-colors">
                    ...
                    <div className="flex justify-between gap-3"><div className="min-w-0"><div className="text-sm font-medium text-ink truncate">{title}</div><div className="text-xs text-ink/60 truncate">{meta}</div></div><div className="text-[11px] text-ink/60 tabular-nums shrink-0">{when}</div></div>
                  </li>
                ))}
              </ol>
            </div>
          ...
          {tab === "booking" && (
            <div className="p-5 md:p-6">
              <div className="flex items-start justify-between gap-4 mb-5">
                <div><div className="text-[11px] font-bold uppercase tracking-[0.12em] text-ink/60">Booking</div><div className="text-lg font-extrabold text-ink tracking-tight mt-1">Brand session · Maya Chen</div><div className="text-sm text-ink/65">Tue, Sep 9 · 2:00 – 4:00 PM · Studio</div></div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-[1fr_220px] gap-5">
                <div>
                  <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-ink/60 mb-3">Where it is</div>
                  <ol className="flex items-center gap-1">
                    {["New lead", "Follow-up", "Booked", "Confirmed", "Complete"].map((s, i) => (
                      <li key={s} className="flex items-center gap-1 flex-1 min-w-0 last:flex-none">
                        <span className={cn("flex items-center gap-1.5 text-[11px] font-bold whitespace-nowrap", i <= 2 ? "text-ink" : i === 3 ? "text-signal-text" : "text-ink/60")}>
                  ...
                  <div className="mt-5 text-[10px] font-bold uppercase tracking-[0.12em] text-ink/60 mb-2">On this booking</div>
                  <ol className="relative pl-6 space-y-2 text-sm">
                    <span aria-hidden className="absolute left-[5px] top-2 bottom-2 w-px bg-border" />
                    <li className="relative"><span aria-hidden className="absolute -left-6 top-[5px] w-[11px] h-[11px] rounded-full border-2 border-white bg-accent" />Maya: “Running 10 late!!” <span className="text-ink/60 text-xs">· Messages</span></li>
                ...
                <div className="rounded-2xl border border-border bg-paper/60 p-4">
                  <div className="text-[10px] font-bold uppercase tracking-[0.12em] text-ink/60 mb-2">On your calendar</div>
          ...
          {tab === "calendar" && (
            <div className="p-5 md:p-6">
              <div className="flex items-center justify-between gap-3 mb-4">
                <div><div className="text-[11px] font-bold uppercase tracking-[0.12em] text-ink/60">Today</div><div className="text-lg font-extrabold text-ink tracking-tight mt-0.5">Tuesday, Sep 9</div></div>
              ...
              <ol className="rounded-2xl border border-border divide-y divide-border overflow-hidden">
                {[
                  ...
                  <li key={String(who)} className={cn("flex items-center gap-4 px-4 py-3 transition-colors", booking ? "hover:bg-black/[0.02]" : "bg-paper/50")}>
                    <div className="w-14 shrink-0 text-right"><div className="text-sm font-semibold text-ink tabular-nums">{String(s)}</div><div className="text-[10px] text-ink/60 tabular-nums">{String(e)}</div></div>
                    <span className={cn("w-1 h-9 rounded-full shrink-0", String(dot))} aria-hidden />
                    <div className="min-w-0 flex-1"><div className={cn("text-sm truncate", booking ? "font-semibold text-ink" : "font-medium text-ink/70")}>{String(who)}</div><div className="text-xs text-ink/65 truncate">{String(what)}</div></div>
                    {booking && <span className="text-[10px] font-bold text-ink/60">Open thread →</span>}
                  </li>
                ))}
              </ol>
              <p className="mt-3 text-xs text-ink/60">Busy time from Google or Apple Calendar blocks booking slots; bookings made here are mirrored back.</p>
            </div>
          )}
          ...
          {tab === "ai" && (
            <div className="p-5 md:p-6 flex flex-col min-h-[400px]">
              <div className="flex items-center gap-2 mb-4"><span className="w-7 h-7 rounded-full bg-signal-soft text-signal-text flex items-center justify-center"><Sparkles className="w-3.5 h-3.5" strokeWidth={2} /></span><span className="text-sm font-extrabold text-ink">Assistant</span><span className="text-[11px] text-ink/60">knows your whole thread</span></div>
              ...
              {!asked && <p className="text-xs text-ink/60">Pick a question. Answers come from your real inbox, calendar and bookings.</p>}
            </div>
          )}
        </div>
      </div>
