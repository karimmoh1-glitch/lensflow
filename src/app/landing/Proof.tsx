  ...
  return (
    <div className="max-w-[1200px] mx-auto px-6">
      <Reveal className="max-w-2xl mb-10">
        <p className="text-2xs font-bold uppercase tracking-[0.16em] text-ink/65 mb-4">Who it&rsquo;s for</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {FOR.map((f) => (
          <div key={f.title} className="rounded-2xl border border-border bg-white px-5 py-5">
      ...
      {PROOF.length > 0 ? (
        <ul className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4" aria-label="From customers">
          {PROOF.map((p) => (
            <li key={p.name + p.business} className="rounded-2xl border border-border bg-paper px-5 py-5">
        ...
        <div className="mt-6 rounded-2xl border border-dashed border-border bg-paper/60 px-5 py-5 flex flex-col sm:flex-row sm:items-center gap-4">
