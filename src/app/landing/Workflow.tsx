  ...
  return (
    <div ref={ref} className="max-w-[1200px] mx-auto px-6">
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)] gap-10 lg:gap-16 items-center">
        <Reveal>
          <p className="text-2xs font-bold uppercase tracking-[0.16em] text-signal-text mb-4">Runs itself</p>
          ...
          <p className="mt-5 text-ink/65 text-base max-w-xs">Message → understand → act → follow up. Written as a sentence you can read. Scroll, and watch it fire.</p>
        </Reveal>
          ...
          <div className="absolute -top-5 left-4 z-10 w-[min(280px,80%)]" style={{ opacity: arrive, transform: `translateY(${(1 - arrive) * 14}px)` }}>
            <div className="flex items-center gap-2.5 rounded-2xl border border-border bg-white shadow-popover px-3 py-2.5">
              ...
              <div className="min-w-0">
                ...
                <div className="text-2xs text-ink/65 truncate">Brand session · Fri 2:30 PM</div>
              </div>
              <span className="ml-auto w-2 h-2 rounded-full bg-accent shrink-0" style={{ opacity: when < 1 ? 1 : 0.3 }} />
            </div>
          </div>

          <div className="rounded-2xl border border-border bg-white shadow-[0_32px_80px_-32px_rgba(16,17,20,0.3)] overflow-hidden pt-8">
            <div className="px-5 py-4 border-b border-border">
              <div className="flex items-center justify-between gap-3 mb-3">
                <span className="text-sm font-semibold text-ink">Booking confirmation</span>
                <span className="flex items-center gap-1.5 text-2xs font-bold text-success-text"><span className="w-1.5 h-1.5 rounded-full bg-success" />On</span>
              </div>
            ...
            <div className="px-5 py-3">
              <div className="text-2xs font-bold uppercase tracking-[0.12em] text-ink/65 mb-1">Recently ran</div>
              <ol className="relative pl-6">
                ...
                <li className="relative" style={{ opacity: sent }}>
                  ...
                  <div className="flex items-start justify-between gap-3 overflow-hidden" style={{ maxHeight: sent * 52, transform: `translateY(${(1 - sent) * -8}px)` }}>
                    <div className="min-w-0 py-2"><div className="text-sm font-medium text-ink truncate">Booking confirmation</div><div className="text-xs text-ink/65 truncate">Sent · Sarah Kim · WhatsApp</div></div>
                    <div className="text-2xs font-bold text-success-text shrink-0 py-2">just now</div>
                  </div>
                </li>
                {[
                  ...
                  <li key={t + w} className="relative py-2 flex items-start justify-between gap-3">
                    ...
                    <div className="min-w-0"><div className="text-sm font-medium text-ink truncate">{t}</div><div className="text-xs text-ink/65 truncate">{m}</div></div>
                    <div className="text-2xs text-ink/65 shrink-0">{w}</div>
                  </li>
                ))}
              </ol>
            </div>
