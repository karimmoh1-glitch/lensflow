        ...
        <div className="space-y-3 max-h-80 overflow-y-auto scrollbar-thin">
          {messages.map((m) => (
            <div key={m.id} className={cn("max-w-[85%]", m.direction === "INBOUND" ? "ml-auto" : "")}>
              <div className={cn("rounded-xl px-3.5 py-2 text-sm", m.direction === "INBOUND" ? "bg-ink text-white" : "bg-ink/[0.05] text-ink")}>
                {m.body}
              </div>
              <div className={cn("text-xs text-ink/65 mt-1", m.direction === "INBOUND" ? "text-right" : "")}>
