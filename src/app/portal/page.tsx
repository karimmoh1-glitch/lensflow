  ...
  return (
    <div className="max-w-2xl mx-auto px-4 md:px-8 py-8 md:py-10">
      ...
      <div className="mb-8">
        ...
        {upcoming.length === 0 ? (
          <EmptyState title="Nothing upcoming" description="Once a booking is confirmed, it'll show up here with the date and details." />
        ) : (
          <Card>
              ...
              {past.map((b) => (
                <div key={b.id} className="px-4 py-3.5">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <div className="text-sm font-medium">{b.service.name}</div>
                      <div className="text-xs text-ink/70">{format(toZonedDisplayDate(b.startAt, business.timezone), "MMM d, yyyy")}</div>
                    </div>
                    <Badge tone={STATUS_TONE[b.status] ?? "neutral"}>{b.status.replaceAll("_", " ").toLowerCase()}</Badge>
                  </div>
                  {b.deliveryUrl && (
