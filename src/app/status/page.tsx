  ...
  return (
    <main className="min-h-screen bg-paper">
      <div className="max-w-2xl mx-auto px-6 py-12 md:py-16">
        ...
        <p className="mt-2 text-sm text-ink/65">Checked when you opened this page, at {checkedAt.toUTCString()}.</p>
          ...
          {core.map((row) => (
            <div key={row.name} className="flex items-start gap-3 px-5 py-4">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-ink/65 mt-0.5">{row.note}</p>
              </div>
            </div>
          ))}
        </section>

        <h2 className="mt-10 text-[11px] font-bold uppercase tracking-[0.14em] text-ink/60">Channels and services</h2>
        <p className="mt-2 text-sm text-ink/65">Daythread doesn&rsquo;t monitor these providers&rsquo; uptime. Each row says whether the connection is configured on this deployment, and links to the provider&rsquo;s own status page.</p>
          ...
          {providers.map((p) => (
            <div key={p.name} className="flex items-start gap-3 px-5 py-4">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-sm font-semibold text-ink">{p.name} <span className={cn("ml-1 text-xs font-bold", p.configured ? "text-success-text" : "text-ink/60")}>{p.configured ? "Configured" : "Not configured"}</span></div>
                <p className="text-xs text-ink/65 mt-0.5">{"note" in p && p.note ? p.note : p.configured ? "Connections can be made from Settings → Channels." : "Not available on this deployment yet; the app says so wherever it would be offered."}</p>
              </div>
              <a href={p.status} target="_blank" rel="noreferrer" className="text-xs font-semibold text-ink/65 hover:text-ink whitespace-nowrap">Provider status ↗</a>
            </div>
          ))}
        </section>

        <p className="mt-8 text-xs text-ink/60">Something wrong that this page doesn&rsquo;t show? Email <a href="mailto:support@daythread.org" className="font-semibold text-ink">support@daythread.org</a>.</p>
      </div>
    </main>
  );
}
