    ...
    return (
      <AuthShell eyebrow="No inbox yet" title="This account isn't part of an inbox yet." lede="Ask whoever invited you for a new link, or start your own.">
        <div className="flex flex-col gap-3">
          <a href="/start" className="inline-flex items-center justify-center h-11 px-5 rounded-lg bg-ink text-white text-sm font-semibold transition-transform">
            Start my own inbox
          </a>
          <form action={logout}>
        ...
        {memberships.map((m) => (
          <li key={m.businessId}>
            <form action={switchWorkspace.bind(null, m.businessId)}>
              <button
                type="submit"
                className="group w-full flex items-center gap-3 px-4 py-3.5 text-left rounded-xl border border-border bg-white transition-all duration-200 ease-[cubic-bezier(0.16,1,0.3,1)] hover:border-ink/25 hover:-translate-y-0.5 hover: focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink/70"
              >
                <span className="w-10 h-10 rounded-full bg-accent-soft text-accent-text flex items-center justify-center text-xs font-semibold shrink-0">{initials(m.business.name)}</span>
