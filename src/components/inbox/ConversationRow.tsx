  ...
  const body = (
    <>
      ...
      <div className={cn("flex items-center gap-3 mb-1", tools && "md:group-hover:pr-32 md:group-focus-within:pr-32")}>
        <div className={cn("relative w-9 h-9 rounded-full flex items-center justify-center text-[11px] font-bold shrink-0 transition-colors", isPerson ? "bg-accent-soft text-accent-text" : "bg-black/[0.05] text-ink/50")}>
          {initials(name)}
        ...
        {!isPerson && categoryLabel && <span className="text-[10px] font-bold rounded-full px-1.5 py-0.5 shrink-0 bg-black/[0.05] text-ink/55">{categoryLabel}</span>}
        ...
        <time dateTime={timeISO} suppressHydrationWarning className={cn("text-[11px] shrink-0 tabular-nums", unread ? "text-ink/70 font-semibold" : "text-ink/45")}>{time}</time>
      </div>
      ...
      <p className={cn("text-[13px] leading-snug line-clamp-2 pl-12", isPerson ? (unread ? "text-ink/85" : "text-ink/65") : "text-ink/55")}>
