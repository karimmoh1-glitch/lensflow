      ...
      {options.map(([k, text]) => {
        ...
        return (
          <button
            ...
            onClick={() => onChange(k)}
            className={cn(
              "group flex items-center gap-3 rounded-xl border bg-white text-left transition-[border-color,background-color,transform,box-shadow] duration-150 active:translate-y-px focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ink/70 focus-visible:ring-offset-2",
          ...
          >
            <span aria-hidden className={cn("shrink-0 w-5 h-5 border flex items-center justify-center transition-colors", multi ? "rounded-[5px]" : "rounded-full", on ? "bg-ink border-ink text-white" : "border-ink/25 bg-white")}>
              {on && <Check className="w-3 h-3" strokeWidth={3.5} />}
            </span>
            <span className={cn("font-medium text-ink leading-snug", size === "md" ? "text-[15px]" : "text-sm")}>{text}</span>
          </button>
        );
      })}
    </div>
