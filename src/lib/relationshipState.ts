...
function waiting(state: RelationshipState, i: RelationshipInput): { theyWaitFor: string | null; youWaitFor: string | null } {
  switch (state) {
    ...
    case "BOOKED":
      return (i.outstandingCents ?? 0) > 0
        ? { theyWaitFor: null, youWaitFor: `$${((i.outstandingCents ?? 0) / 100).toLocaleString()} still owed` }
    ...
    case "COMPLETED":
      return (i.outstandingCents ?? 0) > 0 ? { theyWaitFor: null, youWaitFor: `$${((i.outstandingCents ?? 0) / 100).toLocaleString()} still owed` } : { theyWaitFor: "Their photos or deliverables", youWaitFor: null };
    default:
...
function readCore(i: RelationshipInput): CoreRead {
  ...
  if ((i.outstandingCents ?? 0) > 0 && (i.upcomingBooking || i.lastCompletedBooking)) {
    return {
      ...
      nextAction: { label: `Collect $${((i.outstandingCents ?? 0) / 100).toLocaleString()}`, why: i.upcomingBooking ? "Due before the session" : "Still owed after the session" },
      tone: "warning",
    };
  }
  ...
  if (i.relationship === "CUSTOMER") {
    ...
    return { state: "CUSTOMER", label: "Customer", standing: "A customer. Nothing open right now.", nextAction: null, tone: "outcome" };
  }
