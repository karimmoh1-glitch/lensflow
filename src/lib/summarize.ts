...
export function summarizeDeterministically(input: SummaryInput): ConversationSummary {
  ...
  // A reply that never left (not delivered, failed) doesn't put the ball in their court —
  // the inbox still shows the thread as waiting on you, and the summary must agree.
  const lastDelivered = [...input.messages].reverse().find((m) => m.direction === "INBOUND" || !["NOT_DELIVERED", "FAILED"].includes(m.status ?? ""));
  const latestInbound = inbound[inbound.length - 1];
  const latestText = latestInbound ? splitMessage(latestInbound.body).text : "";
  const first = firstName(input.personName, "They");
  const u = understand({
    body: latestText,
