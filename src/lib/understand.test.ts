...
describe("understand", () => {
  ...
  it("once they're on the calendar, an availability or booking ask points at confirming, not another link", () => {
    const u = understand({ body: "Are you free Tuesday afternoon for a brand session?", relationship: "CUSTOMER", hasUpcomingBooking: true, upcomingBookingLabel: "Brand session · Mon, Sep 7 · 9:00 AM", hasOutstandingPayment: false });
    expect(u.intent).toBe("AVAILABILITY");
