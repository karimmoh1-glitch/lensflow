...
describe("automation runner", () => {
  ...
  it("a second automation doing the same job for the same person is skipped — one confirmation, not two", async () => {
    // A copy with the same action (however it got there) must not write or send a second message.
    const copy = await prisma.automation.create({ data: { businessId, name: "Booking confirmation (copy)", trigger: "BOOKING_CREATED", offsetHours: 0, action: "SEND_CONFIRMATION", messageTemplate: "Booked: {{service}} on {{date}}." } });
    await fireAutomationEvent({ businessId, trigger: "BOOKING_CREATED", targetType: "booking", targetId: bookingId });
    const exec = await prisma.automationExecution.findFirst({ where: { automationId: copy.id, targetId: bookingId } });
    expect(exec?.result).toBe("skipped");
    expect(await prisma.message.count({ where: { direction: "OUTBOUND", conversation: { businessId, clientId } } })).toBe(1);
    await prisma.automation.delete({ where: { id: copy.id } });
  });
