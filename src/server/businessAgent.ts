...
import { toZonedDisplayDate, formatMoney } from "@/lib/utils";
import { addDays, addHours, format, subDays, subHours } from "date-fns";
import type { ChannelType } from "@prisma/client";

/**
 * The Daythread Business Agent — Business plan only (enforced by every caller through
 ...
 * waiting on a reply, bookings that aren't confirmed, balances coming due, leads that went
 * quiet, calendars that stopped syncing — and proposes the specific action for each one,
 ...
 */
export type ProposalKind = "reply" | "confirm_booking" | "payment_reminder" | "follow_up" | "reconnect_calendar";
...
export async function buildAgentBrief(businessId: string, now = new Date()): Promise<AgentBrief> {
  ...
  const [waiting, unconfirmed, due, quiet, calendars] = await Promise.all([
    ...
    prisma.booking.findMany({
      where: { businessId, status: { in: ["BOOKED", "DEPOSIT_PAID"] }, startAt: { gt: now, lte: addHours(now, 72) } },
      include: { client: true, service: true },
      orderBy: { startAt: "asc" },
      take: 10,
    }),
    prisma.payment.findMany({
      where: { businessId, status: "AWAITING_CONFIRMATION", method: { not: "CARD" }, booking: { status: { not: "CANCELED" }, startAt: { lte: addDays(now, 7) } } },
  ...
  const first = (name: string | null | undefined) => (name ?? "there").split(" ")[0];
  ...
  for (const p of due) {
    const id = `payment_reminder:${p.id}`;
    if (done.has(id)) continue;
    const start = p.booking ? toZonedDisplayDate(p.booking.startAt, tz) : null;
    const overdue = p.booking ? p.booking.startAt < now : false;
    proposals.push({
      id,
      kind: "payment_reminder",
      title: `${overdue ? "Chase" : "Remind"} ${p.client.name} about ${formatMoney(p.amountCents)}`,
      why: `${p.purpose === "DEPOSIT" ? "Deposit" : "Balance"} for ${p.booking?.service.name ?? "their booking"}${start ? ` ${overdue ? "was due" : "is due"} around ${format(start, "MMM d")}` : ""}, still unconfirmed.`,
      clientName: p.client.name,
      conversationId: null,
      channel: null,
      draft: `Hi ${first(p.client.name)} — a quick note from ${business.name}: the ${p.purpose === "DEPOSIT" ? "deposit" : "balance"} of ${formatMoney(p.amountCents)} for your ${p.booking?.service.name ?? "booking"}${start ? ` on ${format(start, "MMMM d")}` : ""} is still open${p.reference ? ` (reference ${p.reference})` : ""}. Reply here if you've already sent it and I'll mark it received. Thank you!`,
      href: `/dashboard/payments`,
      priority: overdue ? 85 : 70,
  ...
  for (const cal of calendars) {
    proposals.push({
      ...
      channel: null,
      draft: null,
      href: `/dashboard/settings?tab=connections&setup=${cal.provider}`,
      priority: 95,
      valueCents: null,
    });
  }
...
export async function draftForProposal(businessId: string, proposal: AgentProposal): Promise<string | null> {
  ...
  return draftReply({ businessName: business.name, services: services.map((s) => ({ name: s.name, priceCents: s.priceCents, durationMins: s.durationMins })), customerMessage: conversation.messages[0]?.body ?? "", customerName: conversation.client?.name, depositPercent: business.depositPercent });
}
...
export async function executeProposal(params: { businessId: string; userId: string; proposal: AgentProposal; body: string }): Promise<ExecutionResult> {
  ...
  if (proposal.kind === "payment_reminder") {
    const p = await prisma.payment.findFirst({ where: { id: proposal.id.split(":")[1], businessId }, include: { client: true } });
    if (!p) return { ok: false, error: "That payment no longer exists." };
    if (p.status === "PAID") return { ok: false, error: "Already paid — nothing to send." };
    paymentId = p.id;
    clientId = p.clientId;
    if (!conversation) conversation = await prisma.conversation.findFirst({ where: { businessId, clientId: p.clientId, archived: false }, orderBy: { lastMessageAt: "desc" } });
    channel = conversation?.channel ?? (p.client.email ? "EMAIL" : p.client.phone ? "SMS" : null);
    to = conversation?.externalHandle ?? p.client.email ?? p.client.phone ?? null;
  }
  if (!channel || !to) {
  ...
  if (delivery.status === "SENT" && bookingId) await prisma.booking.updateMany({ where: { id: bookingId, businessId, status: { in: ["BOOKED", "DEPOSIT_PAID"] } }, data: { status: "CONFIRMED" } });
  if (paymentId) await track("payment_reminder_sent", { businessId, properties: { paymentId, via: delivery.via } });
