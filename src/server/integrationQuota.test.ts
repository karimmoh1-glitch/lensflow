...
describe("integration quota", () => {
  ...
  it("Pro: six connect, the seventh is rejected", async () => {
    const id = await business("PRO");
    for (const p of QUOTA_PROVIDERS) expect((await connect(id, p)).ok).toBe(true);
    expect((await integrationUsage(id)).active).toBe(6);
    // Every quota provider is now used; a seventh distinct activation can only be modelled
    // by the count check itself — the usage says the plan is full.
    const usage = await integrationUsage(id);
    expect(usage).toMatchObject({ plan: "PRO", active: 6, limit: 6, atLimit: true, nextPlan: "BUSINESS" });
    expect(usageFor({ planTier: "PRO", billingStatus: "ACTIVE" }, ([...QUOTA_PROVIDERS, "EMAIL"] as IntegrationProvider[]).map((provider) => ({ provider, status: "CONNECTED" as const }))).atLimit).toBe(true);
  });
