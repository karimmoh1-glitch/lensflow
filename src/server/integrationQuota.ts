...
import { PLANS, effectivePlan, planForIntegrations, planPurchasable, type PlanKey } from "@/lib/billing";
...
export function usageFor(business: { planTier: PlanKey; billingStatus: string | null; compedPlan?: PlanKey | null; betaProEndsAt?: Date | null }, rows: Array<{ provider: IntegrationProvider; status: IntegrationStatus }>): IntegrationUsage {
  ...
  return { plan, active, limit, atLimit, overQuota: active > limit, nextPlan: next && next !== plan && planPurchasable(next) ? next : null };
}

export async function integrationUsage(businessId: string): Promise<IntegrationUsage> {
  const [business, rows] = await Promise.all([
    prisma.business.findUniqueOrThrow({ where: { id: businessId }, select: { planTier: true, billingStatus: true, compedPlan: true, betaProEndsAt: true } }),
...
export async function activateIntegration(params: {
  ...
  const result = await prisma.$transaction(
    async (tx) => {
      ...
      const locked = await tx.$queryRaw<Array<{ id: string; planTier: PlanKey; billingStatus: string | null; compedPlan: PlanKey | null; betaProEndsAt: Date | null }>>`SELECT "id", "planTier", "billingStatus", "compedPlan", "betaProEndsAt" FROM "Business" WHERE "id" = ${businessId} FOR UPDATE`;
