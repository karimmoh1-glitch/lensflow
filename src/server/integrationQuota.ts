...
export function limitMessage(usage: IntegrationUsage): string {
  ...
  const allowance = `${plan.name} includes ${usage.limit} connected integration${usage.limit === 1 ? "" : "s"}.`;
  return next ? `Integration limit reached. ${allowance} Upgrade to ${next} to connect more.` : `Integration limit reached. ${allowance}`;
}
